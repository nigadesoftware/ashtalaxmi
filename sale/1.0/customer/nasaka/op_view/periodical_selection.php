<?php
    ...
    //
    /* if (isaccessible(123468567)==0 or isaccessible(123471420)==0)
    {
    ...
    <head>
        ...
        <script>
            $(function()
            {
                ...
                source: '../ip_view/purchaser_search.php',
                minLength:0,
                delay:200,
                select:function(event,ui)
                {var v = ui.item.value;
    ...
    <body>
        ...
        <article class="w3-container">
            <?php
                ...
                elseif ($categorycode_de==11)
                {
                    echo '<form method="post" action="../op_controller/invoicedespatch_action.php">';
                }
                elseif ($categorycode_de==12)
                {
                    echo '<form method="post" action="../op_controller/memoinvoicedespatch_action.php">';
                }
                elseif ($categorycode_de==13)
                {
                    echo '<form method="post" action="../op_controller/godowninsurance_action.php">';
                }
                elseif ($categorycode_de==14)
                {
                    echo '<form method="post" action="../op_controller/purchasersaledetail_action.php">';
                }
                elseif ($categorycode_de==15)
                {
                    echo '<form method="post" action="../op_controller/einvoice_action.php">';
                }
                elseif ($categorycode_de==16)
                {
                    echo '<form method="post" action="../op_controller/productionsaleinfo_action.php">';
                }
                echo '<table border="0" >';
                if ($categorycode_de==1 or $categorycode_de==2 or $categorycode_de==3 or $categorycode_de==4 or $categorycode_de==5 or $categorycode_de==7 or $categorycode_de==8 or $categorycode_de==9 or $categorycode_de==11 or $categorycode_de==12 or $categorycode_de==13 or $categorycode_de==14 or $categorycode_de==16) 
                {
                    echo '<tr>';
                ...
                if ($categorycode_de==1 or $categorycode_de==2 or $categorycode_de==3 or $categorycode_de==4 or $categorycode_de==5 or $categorycode_de==6 or $categorycode_de==7 or $categorycode_de==8 or $categorycode_de==9 or $categorycode_de==11 or $categorycode_de==12 or $categorycode_de==13 or $categorycode_de==14 or $categorycode_de==16)
                {
                    echo '<tr>';
                    if ($_SESSION['lng']=="English")
                    {
                    ...
                    if ($categorycode_de==1 or $categorycode_de==2 or $categorycode_de==3 or $categorycode_de==4 or $categorycode_de==5 or $categorycode_de==6 or $categorycode_de==7 or $categorycode_de==8 or $categorycode_de==9 or $categorycode_de==10 or $categorycode_de==16)
                    {
                        $todate = currentdate();
                    }
                    else
                ...
                if ($categorycode_de==6)
                    ...
                    if ($categorycode_de==14)
                    {
                        echo '<tr>';
                        if ($_SESSION['lng']=="English")
                        {
                            echo '<tr>';
                            echo '<td><label for="purchaser">Purchaser</label></td>';
                            echo '</tr>';
                        }
                        else
                        {
                            echo '<tr>';
                            echo '<td><label for="purchaser">खरेदीदार</label></td>';
                            echo '</tr>';
                        }

                        echo '<td><input type="text" style="font-size:12pt;height:30px" name="purchaser" id="purchaser" value =""></td>';
                        echo '<td><label for="purchaser">*</label></td>';
                        echo '</tr>';
                        echo '<tr>';
                        echo '<td><input type="text" readonly="readonly" style="font-size:12pt;height:30px" name="purchasercode" id="purchasercode"></td>';
                        echo '</tr>';
                    }    
