...
class memoinvoicedespatch extends swappreport
{	
    ...
    public function __construct(&$connection,$maxlines,$languagecode=1,$rowheight=7)
	{
        ...
        $this->pdf->SetSubject('Memo Invoice Delivery Report');
        $this->pdf->SetKeywords('MEMINVDEL_000.EN');
        // set font
    ...
    function endreport()
    {
        ...
        $this->pdf->Output('MEMINVDEL_000.pdf', 'I');
    }
	function pageheader()
    {
        $this->liney = 10;
        ...
        //textbox($data,$width,$col,$datatype='',$alighment='',$languagecode='',$fontname='',$fontsize='',$newline='',$cangrow='',$currencysymbol='')  
        $this->newrow(5);
        $this->textbox('Sugar Memo Invoice Despatched',250,10,'S','C',1,'verdana',13);
        $this->newrow(8);
        ...
        $this->textbox('Purchaser',80,100,'S','L',1,'verdana',11);
        $this->textbox('Destination',50,180,'S','L',1,'verdana',11);
        $this->textbox('Rate',30,215,'S','R',1,'verdana',11);
        $this->textbox('Quantity(qtl)',35,235,'S','R',1,'verdana',11);
        $this->textbox('Amount',30,255,'S','R',1,'verdana',11);
    ...
    function detail()
    {
        $query ="select h.transactionnumber,h.purchasercode
        ,h.goodscategorycode,h.brokercode,h.shippingpartycode
        ,m.memonumberpresuf,h.taxableamount
        from saleinvoiceheader h,salememoheader m
        where h.salememotransactionnumber=m.transactionnumber
        ...
        while ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
        {
            ...
            $this->textbox($row['MEMONUMBERPRESUF'],35,10,'S','L',1,'verdana',7);
            $this->textbox($saleinvoiceheader1->invoicenumberpresuf,35,40,'S','L',1,'verdana',7);
            $this->textbox($saleinvoiceheader1->invoicedate,25,75,'S','L',1,'verdana',9);
            $this->textbox($goodspurchaser1->purchasercode.' '.$goodspurchaser1->purchasernameeng,80,100,'S','L',1,'verdana',8);
            $this->textbox($shippingparty1->address,50,180,'S','L',1,'verdana',8);
            $this->textbox($saleinvoiceheader1->rate(),30,220,'S','R',1,'verdana',8);
            $q=$saleinvoiceheader1->quantity();
            $a=$row['TAXABLEAMOUNT'];
            $qty=$qty+$q;
            $amt=$amt+$a;
            $this->textbox($q,30,235,'S','R',1,'verdana',8);
            $this->textbox($a,30,255,'S','R',1,'verdana',8);
            $this->newrow();
            $this->hline(10,300,$this->liney,'C');    
        }
        $this->textbox('Total',30,200,'S','L',1,'verdana',8);
        $this->textbox($qty,30,235,'S','R',1,'verdana',8);
        $this->textbox($amt,30,255,'S','R',1,'verdana',8);
        $this->newrow();
        $this->hline(10,300,$this->liney,'C');    
    }
}    
