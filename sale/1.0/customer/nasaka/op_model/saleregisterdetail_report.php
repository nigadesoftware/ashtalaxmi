...
{	
    ...
    function export()
    {
        ...
        //,(nvl(grossamount,0)+nvl(crcommissionamount,0)+nvl(balcommissionamount,0)) as total
        

            $query ="select h.invoicenumberpresuf
            ,to_char(h.invoicedate,'dd/mm/yyyy') invoicedate
            ,r.memonumberpresuf
            ,s.salecategorynameeng
            ,g.purchasernameeng
            ,b.purchasernameeng brokernameeng
            ,h.vehiclenumber
            ,f.finishedgoodsnameeng
            ,d.productionyearcode,d.salequantity
            ,d.salerate,d.amount
            ,d.cgstamount,d.sgstamount
            ,d.igstamount,d.ugstamount
            ,d.totaltaxamount,d.grossamount
            from saleinvoiceheader h, salecategory s,saleinvoicedetail d,salememoheader r
            ,goodspurchaser g,goodspurchaser b,finishedgoods f
            where h.purchasercode=g.purchasercode
            and h.brokercode=b.purchasercode
            and h.salecategorycode=s.salecategorycode
            and h.transactionnumber=d.transactionnumber
            and d.finishedgoodscode=f.finishedgoodscode
            and h.salememotransactionnumber=r.transactionnumber
            and h.invoicedate>='".$this->fromdate."'
            and h.invoicedate<='".$this->todate."'
            order by invoicenumber";
           ...
           $r = oci_execute($result);
           $response = array();
           $filename='saleregisterdetail.csv';
           //$fp1 = fopen('../../../../../../exportb2b/'.$name.'.csv', 'w');
           $fp1=fopen('php://memory', 'w');

           fputcsv($fp1, array('Inv. No','Inv. Date'
           ,'Memo Number'
           ,'Sale Type','Purchaser','Broker','Vehicle Number'
           ,'Grade','Prod.Season','Quintals'
           ,'Rate','Taxable','CGST'
           ,'SGST','IGST','UGST','Total Tax','Net Amt'));
           while ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
           {
                fputcsv($fp1, 
                array($row['INVOICENUMBERPRESUF']
                ,$row['INVOICEDATE']
                ,$row['MEMONUMBERPRESUF']
                ,$row['SALECATEGORYNAMEENG']
                ,$row['PURCHASERNAMEENG']
                ,$row['BROKERNAMEENG']
                ,$row['VEHICLENUMBER']
                ,$row['FINISHEDGOODSNAMEENG']
                ,$row['PRODUCTIONYEARCODE']
                ,$row['SALEQUANTITY']
                ,$row['SALERATE']
                ,$row['AMOUNT']
                ,$row['CGSTAMOUNT']
