...
{	
	...
	function pageheader()
    {
        ...
        $this->pdf->Image('@'.$img,165,20,40,40);
        $this->textbox('Invoice Number : '.$saleinvoiceheader1->invoicenumberpresuf,80,10,'S','L',1,'verdana',10,'','','','B');
        $this->textbox('Ack No : '.$saleinvoiceheader1->acknumber,70,110,'S','L',1,'verdana',10);
        $this->newrow();
        $this->textbox('Invoice Date : '.$saleinvoiceheader1->invoicedate,80,10,'S','L',1,'verdana',9);
        $this->textbox('Ack Date : '.$saleinvoiceheader1->ackdate,70,110,'S','L',1,'verdana',9);
        $this->newrow();
        //$this->irnnumber = $row['IRNNUMBER'];
        //$this->acknumber = $row['ACKNUMBER'];
        //$this->ackdate = $row['ACKDATE'];
        $this->textbox('IRN :',22,10,'S','L',1,'verdana',10);
        $this->textbox($saleinvoiceheader1->irnnumber,140,20,'S','L',1,'verdana',9);
        $this->newrow(5);
        $this->textbox('Memo Number : '.$salememoheader1->memonumberpresuf,80,10,'S','L',1,'verdana',9,'','','','');
        $this->textbox('Memo Date : '.$salememoheader1->memodate,80,110,'S','L',1,'verdana',9);
        $this->newrow(5);
        $this->textbox('FSSAI L No. : 11522999000366',80,10,'S','L',1,'verdana',9,'','','','');
        $this->newrow(7);
        $this->liney = 60;
        ...
        $this->textbox('HSNCODE : 170114',40,110,'S','L',1,'verdana',10);
        $this->newrow();

        $this->liney = 95;
        $this->textbox('Sr.',10,10,'S','L',1,'verdana',10,'','','','B');
        $this->textbox('Description of Goods',50,20,'S','L',1,'verdana',10,'','','','B');
        $this->textbox('Qty',20,70,'S','R',1,'verdana',10,'','','','B');
        $this->textbox('Rate',15,90,'S','R',1,'verdana',10,'','','','B');
        $this->textbox('Taxable Value',30,110,'S','R',1,'verdana',10,'','','','B');
        if ($this->statecode == 27)
        {
            $this->textbox('CGST',30,130,'S','R',1,'verdana',10,'','','','B');
            $this->textbox('SGST',30,160,'S','R',1,'verdana',10,'','','','B');
        }
        elseif (in_array($statecode, array(4,7,25,26,31,34,35)))
        {
            $this->textbox('IGST',30,130,'S','R',1,'verdana',10,'','','','B');
            $this->textbox('UGST',30,160,'S','R',1,'verdana',10,'','','','B');
        }
        ...
        $this->newrow();
        $this->textbox('Qtls',10,80,'S','R',1,'verdana',9,'','','','B');
        $this->textbox('%',10,137,'S','R',1,'verdana',9,'','','','B');
        $this->textbox('Amount',20,147,'S','R',1,'verdana',9,'','','','B');
        $this->textbox('%',15,162,'S','R',1,'verdana',9,'','','','B');
        $this->textbox('Amount',20,177,'S','R',1,'verdana',9,'','','','B');
    ...
    function detail()
    {
        ...
        while ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
        {
            ...
            $this->textbox($row['SALEQUANTITY'],15,75,'S','R',1,'verdana',9);
            $this->textbox($this->moneyFormatIndia($row['SALERATE']),20,90,'S','R',1,'verdana',9);
            $this->textbox($this->moneyFormatIndia($row['AMOUNT']),30,110,'S','R',1,'verdana',9);
            if ($this->statecode == 27)
            {
            ...
            $this->newrow(4);
            $this->textbox('Prod Year-'.$row['PERIODNAME_ENG'],50,20,'S','L',1,'verdana',9);
            $this->newrow(5);
         }
        ...
        $saleinvoiceheader1->fetch();
        $this->newrow(2);
        $this->textbox('Total Taxable Amount',50,140,'S','L',1,'verdana',9,'','','','B');
        ...
        {
            $this->textbox('Total CGST Amount',50,140,'S','L',1,'verdana',9,'','','','B');
        ...
        {
            $this->textbox('Total SGST Amount',50,140,'S','L',1,'verdana',9,'','','','B');
        ...
        {
            $this->textbox('Total IGST Amount',50,140,'S','L',1,'verdana',9,'','','','B');
        ...
        {
            $this->textbox('Total UGST Amount',50,140,'S','L',1,'verdana',9,'','','','B');
            $this->textbox($this->moneyFormatIndia($saleinvoiceheader1->ugstamount),30,170,'S','R',1,'verdana',9);
            $this->newrow();
            $this->hline(140,200,$this->liney+$limit);
        }
        if ($saleinvoiceheader1->tcs>0)
        {
            $this->textbox('+TCS',50,140,'S','L',1,'verdana',9);
            $this->textbox($this->moneyFormatIndia($saleinvoiceheader1->tcs),30,170,'S','R',1,'verdana',9);
            $this->newrow();
            $this->hline(140,200,$this->liney+$limit);
        }
        elseif ($saleinvoiceheader1->tds>0)
        {
            $this->textbox('-TDS',50,140,'S','L',1,'verdana',9);
            $this->textbox($this->moneyFormatIndia($saleinvoiceheader1->tds),30,170,'S','R',1,'verdana',9);
            $this->newrow();
            $this->hline(140,200,$this->liney+$limit);
        }
        $this->textbox('Total  Amount',50,140,'S','L',1,'verdana',9,'','','','B');
        ...
        $this->newrow(10);
        $this->textbox('2. Error and mission in this invoice shall be subjected to the jurisdiction of the Nashik',135,10,'S','L',1,'verdana',9,'','Y');
        $this->liney = 207;
        ...
        {
            $paise = round(($saleinvoiceheader1->grossamount-intval($saleinvoiceheader1->grossamount))*100);
            $paise = $paise%100;
            $wrd1 = getStringOfAmount(intval($saleinvoiceheader1->grossamount));
            $wrd2 = getStringOfAmount($paise);
            $wrd = 'Rs '.$wrd1.' and '.$wrd2.' Paise Only';
            $this->textbox($wrd,180,50,'S','L',1,'siddhanta',7,'','Y');
        }
        $this->newrow(5);
        ...
        {
            $this->textbox('By Road',45,160,'S','L',1,'verdana',9);
        }
        else
        {
            $this->textbox('By Rail',45,160,'S','L',1,'verdana',9);
        }
        $this->newrow();
        ...
        if ($saleinvoiceheader1->salecategorycode == 3)
        {
            //$this->textbox('Export Under Quota of MAEQ Sugar Mill: Nashik Sahakari Sakhar Karkhana Ltd, Plant Name: Palase, Plant Code: 12001 At Post: Rajaram Nagar, Taluka: DINDORI Dist: NASHIK, PIN-422209, Maharashtra State, India.  GSTIN: 27AAXCA2984P1ZJ (as per GOI (Dept. of Food & Pub. Distribution) Circular File No.1(6)/2020-SP.I dated 31.12.2020) EXPORT UNDER CONCESSIONAL RATE OF DUTY AS PER NOTI. 40/2017 & 41/2017 DT.23.10.17',120,10,'S','L',1,'verdana',7,'','Y');
            $this->textbox('Export Under OGL (Open General Licence) Date.-27/11/2021',120,10,'S','L',1,'verdana',7,'','Y');
        }

        $this->newrow(10);    
        $this->textbox('For Astalaxmi Sugar Ethanol And Energy Limited (Unit 1)',100,100,'S','L',1,'verdana',9);  
        $this->newrow(10);    
