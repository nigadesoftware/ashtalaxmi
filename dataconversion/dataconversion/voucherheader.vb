...
Public Partial Class voucherheader
	...
	Sub convertheader()
		Try 
        	Dim sql As String = "select h.*,case when nvou_sub_type<>7 then vouchernumberbasevalue(h.dvou_date,2) else vouchernumberbasevalue(h.dvou_date,4) end as voucherbase from ac_voucher_header@erpold h where vfin_year='2019-2020' and nvou_sub_type<>7 and nvou_status=9 order by ntrans_no"
			StartConnection()
			Dim cmd As New OracleCommand(sql, conn)
			...
			While dr.Read()
				...
				cmd.CommandText = "insert into voucherheader(transactionnumber, legalentitycode, yearperiodcode, vouchersubtypecode, vouchernumberbasevalue, vouchernumber, vouchernumberprefixsufix, voucherdate, description, narration, approvalstatus, vouchernumberseriesid)" & _
					"	values (20192020*1000000+" + dr("ntrans_no").ToString()+"+200000" + ",1,20192020," + vousubtype.ToString() + ",'" + dr("voucherbase").ToString() + "'," + dr("nvou_number").ToString() + ",'" + vouwithprefix.ToString() + "','" + Convert.ToDateTime(dr("dvou_date")).ToString("dd-MMM-yyyy") + "'," + lblDescription.Text + "," + lblNarration.Text  + ",9," + numberseries(vousubtype).ToString() + ")"
				cmd.ExecuteNonQuery()
			...
			"And h.vfin_year='2019-2020' And h.nvou_status=9 and t.nac_code=m.refcode  and t.vind_code is null union all Select t.*,m.accountcode,s.subledgercode,h.nvou_sub_type from ac_voucher_header@erpold h "  & _ 
			",ac_voucher_detail@erpold t,accounthead m ,accountsubledger s where h.ntrans_no=t.ntrans_no And h.vfin_year='2019-2020' And h.nvou_status=9 and t.nac_code=m.refcode and m.accountcode=s.accountcode "  & _ 
			...
			While dr1.Read()
				...
				cmd1.CommandText = "insert into voucherdetail(transactionnumber, detailserialnumber,accountcode,subledgercode,credit,debit)" & _
					"	values (20192020*1000000+" + dr1("ntrans_no").ToString()+"+200000" + "," + srno.ToString() + "," + dr1("accountcode").ToString() + "," + indcode + "," + dr1("ncr_amt").ToString() + "," + dr1("ndr_amt").ToString() + ")"
				qr=cmd1.CommandText
			...
			If (iscashentry = True) Then	
				If (drcr = "Dr") Then
					cmd1.CommandText = "insert into voucherdetail(transactionnumber, detailserialnumber,accountcode,subledgercode,credit,debit)" & _
					"	values (20192020*1000000+" + dr("ntrans_no").ToString()+"+200000" + "," +srno.ToString() + ",1200001,null,0," + amt.ToString() + ")"	
				Else	
					cmd1.CommandText = "insert into voucherdetail(transactionnumber, detailserialnumber,accountcode,subledgercode,credit,debit)" & _
					"	values (20192020*1000000+" + dr("ntrans_no").ToString()+"+200000" + "," + srno.ToString() + ",1200001,null," + amt.ToString() + ",0)"
				End If
			...
			While dr2.Read()
				...
				cmd2.CommandText = "insert into voucherchequedddetail(transactionnumber, detailserialnumber,bankaccountcode,funddocumentcode,funddocumentnumber,funddocumentdate,funddocumentamount,draweebank)" & _
					"	values (20192020*1000000+" + dr("ntrans_no").ToString()+"+200000" + "," + srno1.ToString() + "," + bankaccountcode.ToString() + "," + type.ToString() +"," + dr2("nnumber").ToString() + ",'" + Convert.ToDateTime(dr2("ddate")).ToString("dd-MMM-yyyy") + "'," + dr2("namount").ToString() + "," + lblNarration.Text + ")"
				qr=cmd2.CommandText
