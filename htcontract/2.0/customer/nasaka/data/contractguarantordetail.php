<?php
    require("../info/phpsqlajax_dbinfo.php");
    require("../info/phpgetlogin.php");
    include("../info/ncryptdcrypt.php");
    require("../info/rawmaterialroutine.php");
    //Raw Material Transaction Addition or Alteration
    if (isaccessible(451230287895415)==0 and isaccessible(475124562358965)==0)
    {
        echo 'Communication Error';
        exit;
    }
    $contractid_de = fnDecrypt($_GET['contractid']);
    $servicecontractorcategoryid_de = fnDecrypt($_GET['servicecontractorcategoryid']);
    if (isset($_GET['contractguarantordetailid']))
    {
        $contractguarantordetailid_de = fnDecrypt($_GET['contractguarantordetailid']);    
    }
    if ($servicecontractorcategoryid_de == 452168578)
    {
        $iscultivator = 0;
        echo '<input type="hidden" style="font-size:12pt;height:30px" name="iscultivator" id="iscultivator" style="width:300px" value ="'.$iscultivator.'">';
    }
    elseif ($servicecontractorcategoryid_de == 324152658)
    {
        $iscultivator = 1;
        echo '<input type="hidden" style="font-size:12pt;height:30px" name="iscultivator" id="iscultivator" style="width:300px" value ="'.$iscultivator.'">';
    }
    $flag = $_GET['flag'];
    // Opens a connection to a MySQL server
    $connection=rawmaterial_connection();
?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8"></meta>
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="stylesheet" href="../css/swapp123.css">
        <title>Contract Gurantor Detail</title>
        <style type="text/css">
            @font-face {
            font-family: siddhanta;
            src: url("../fonts/siddhanta.ttf");
            font-weight: normal;
            }
            body
            {
                background-color: #fff;
            }
            header
            {
                background-color: #fff;
                min-height: 38px;
                color: #070;
                font-family: Arial;
                font-size: 19px;
            }
            nav
            {
                width: 300px;
                float: left;
                list-style-type: none;
                font-family: verdana;
                font-size: 15px;
                color: #f48;
                line-height: 30px;
            }
            a
            {
                color: #f48;
            }
            article
            {
                background-color: #fff;
                display: table;
                margin-left: 0px;
                padding-left: 10px;
                font-family: Verdana;
                font-size: 15px;
            }
            section
            {
                margin-left: 0px;
                margin-right: 15px;
                float: left;
                text-align: justify;
                color: #000;
                line-height: 23px;
            }
            footer
            {
                float: bottom;
                color: #000;
                font-family: verdana;
                font-size: 12px;
            }
            div
            {
                float:left;
            }
            input, textarea
            {
                outline: none;
                font-family: siddhanta;
            }
            button
            {
                width:200px;
                height:35px;
                color:#000;
                border-radius: 5px;
            }
            input:focus, textarea:focus
            {
                border-radius: 5px;
                outline: none;
                font-family: siddhanta;
                background-color: #fef;
            }
            label
            {
                color: #333;
                font-family: siddhanta;
                font-size: 18px;
                font-weight: normal;
            }
        </style>
        <link rel="stylesheet" href="../css/ui/1.11.4/themes/smoothness/jquery-ui.css">
        <script src="../js/jquery-1.10.2.js"></script>
        <script src="../js/ui/1.11.4/jquery-ui.js"></script>
        <!-- <script src="../js/1.11.0/jquery.min.js"> -->
         <script>
            $(function()
            {
                $("#servicecontractor").autocomplete({
                source: 'servicecontractor_search.php?iscultivator='+$('#iscultivator').val(),
                minLength:2,
                delay:200,
                select:function(event,ui)
                {var v = ui.item.value;
                 var i = ui.item.id;
                $('#servicecontractorid').val(i);
                this.value = v;
                return false;}
                });
            });
        </script>
         <script>
            $(document).ready(function(){
             setInterval(function(){cache_clear()},3600000);
             });
             function cache_clear()
            {
             window.location.reload(true);
            }
        </script>
    </head>
    <body>
        <nav "w3-container">
            <ul class="navbar">
                <li><a class="navbar" href="../data/entitymenu.php">Entity Menu</a><br/>
                <?php
                    /*$personnamedetailtypeid_en = fnEncrypt($personnamedetailtypeid_de);
                    $personnamedetailid_en = fnEncrypt($personnamedetailid_de);
                    echo '<li><a style="color:#f48;text-align:left;" href="../data/personnamedetail_find.php?personnamedetailtypeid='.$personnamedetailtypeid_en.'">personnamedetail Find</a><br/>';*/
                    echo '<li><a class="navbar" href="../data/contract.php?contractid='.fnEncrypt($contractid_de).'&flag='.fnEncrypt('Display').'">Add/Display Contract</a></br>';
                    echo '<li><a class="navbar" href="../data/contractguarantordetail_list.php?contractid='.fnEncrypt($contractid_de).'&flag='.fnEncrypt('Display').'">Add/Display Contract Harvester Detail List</a></br>';
                    echo '<li><a style="color:#f48" class="navbar" href="../../../../../sqlproc/logout.php">Log Out</a><br/>';
                ?>
            </ul>
        </nav>
        <article class="w3-container">
            <div><img src="../img/contract.png" width="201" height="41px"></div>
            <?php
                if ($servicecontractorcategoryid_de == 452168578)
                {
                    $query = "select f.*,s.name_eng as contractorname_eng,s.name_unicode as contractorname_unicode from contractguarantordetail f, servicecontractor s where f.active=1 and f.servicecontractorid=s.servicecontractorid and f.contractid = ".$contractid_de." and f.contractguarantordetailid=".$contractguarantordetailid_de;
                }
                elseif ($servicecontractorcategoryid_de == 324152658)
                {
                    $query = "select f.*,c.name_eng as contractorname_eng,c.name_unicode as contractorname_unicode from contractguarantordetail f, cultivator c where f.active=1 and f.servicecontractorid=c.cultivatorid and f.contractid = ".$contractid_de." and f.contractguarantordetailid=".$contractguarantordetailid_de;
                }
                //echo $query;
                $result = oci_parse($connection, $query); $r = oci_execute($result);
                $r = oci_execute($result);
                //$result = oci_parse($connection, $query); $r = oci_execute($result);
                if ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
                {
                    echo '<section>';
                    //if ($flag_de == 'Display')
                    //{
                        echo '<form method="post" action="../api_action/contractguarantordetail_action.php">';
                    //}
                        echo '<table border="0" >';

                        echo '<tr>';  
                        echo '<td></td>';  
                        echo '</tr>';

                        if ($_SESSION['lng']=="English")
                        {
                            echo '<tr>';
                            echo '<td><label for="servicecontractorid">Service Contractor</label></td>';
                            echo '</tr>';
                        }
                        else
                        {
                            echo '<tr>';
                            echo '<td><label for="servicecontractorid">सेवा कंत्राटदार</label></td>';
                            echo '</tr>';
                        }
                        echo '<tr>';
                        if ($_SESSION['lng']=="English")
                        {
                            echo '<td><input type="text" style="font-size:12pt;height:30px" name="servicecontractor" id="servicecontractor" style="width:300px" value="'.$row['CONTRACTORNAME_ENG'].'"></td>';
                        }
                        else
                        {
                            echo '<td><input type="text" style="font-size:12pt;height:30px" name="servicecontractor" id="servicecontractor" style="width:300px" value="'.$row['CONTRACTORNAME_UNICODE'].'"></td>';
                        }
                        echo '<td><label for="servicecontractorid">*</label></td>';
                        echo '</tr>';
                        echo '<tr>';
                        echo '<td><input readonly="readonly" type="text" style="font-size:12pt;height:30px" name="servicecontractorid" id="servicecontractorid" value="'.$row['SERVICECONTRACTORID'].'"></td>';
                        echo '</tr>';

                        if ($iscultivator == 1)
                        {
                            if ($_SESSION['lng']=="English")
                            {
                                echo '<tr>';
                                echo '<td><label for="fieldarea">Field Area</label></td>';
                                echo '</tr>';
                            }
                            else
                            {
                                echo '<tr>';
                                echo '<td><label for="fieldarea">क्षेत्र</label></td>';
                                echo '</tr>';
                            }

                            echo '<tr>';
                            echo '<td><input type="text" style="font-size:12pt;height:30px" name="fieldarea" id="fieldarea" style="width:300px" value="'.$row['FIELDAREA'].'"></td>';
                            echo '<td><label for="fieldarea">*</label></td>';
                            echo '</tr>';
                            if ($_SESSION['lng']=="English")
                            {
                                echo '<tr>';
                                echo '<td><label for="plantationarea">Plantation Area</label></td>';
                                echo '</tr>';
                            }
                            else
                            {
                                echo '<tr>';
                                echo '<td><label for="plantationarea">लागवड क्षेत्र</label></td>';
                                echo '</tr>';
                            }

                            echo '<tr>';
                            echo '<td><input type="text" style="font-size:12pt;height:30px" name="plantationarea" id="plantationarea" style="width:300px" value="'.$row['PLANTATIONAREA'].'"></td>';
                            echo '<td><label for="plantationarea">*</label></td>';
                            echo '</tr>';
                        }

                        echo '<tr>';
                        if ($_SESSION['lng']=="English")
                        {
                            echo '<tr>';
                            echo '<td><label for="address">Address</label></td>';
                            echo '</tr>';
                        }
                        else
                        {
                            echo '<tr>';
                            echo '<td><label for="address">पत्ता</label></td>';
                            echo '</tr>';
                        }
                        echo '<td><input type="text" style="font-size:12pt;height:30px" name="address" id="address" value="'.$row['ADDRESS'].'"></td>';
                        echo '<td><label for="address">*</label></td>';
                        echo '</tr>';

                        echo '<tr>';
                        echo '<td><input type="hidden" style="font-size:12pt;height:30px" name="contractid" id="contractid" style="width:300px" value ="'.$row['CONTRACTID'].'"></td>';
                        echo '<td><input type="hidden" style="font-size:12pt;height:30px" name="contractguarantordetailid" id="contractguarantordetailid" style="width:300px" value ="'.$row['CONTRACTGUARANTORDETAILID'].'"></td>';
                        echo '</tr>';
                        if ($servicecontractorcategoryid_de == 452168578)
                        {
                            echo '<input type="hidden" style="font-size:12pt;height:30px" name="iscultivator" id="iscultivator" style="width:300px" value ="'.$iscultivator.'">';
                        }
                        elseif ($servicecontractorcategoryid_de == 324152658)
                        {
                            echo '<input type="hidden" style="font-size:12pt;height:30px" name="iscultivator" id="iscultivator" style="width:300px" value ="'.$iscultivator.'">';
                        }
                        if ($flag=='change')
                        {
                            echo '<tr>';
                            echo '<td><input type="submit" name="btn" value="Change" style="width:100px"></td>';
                            echo '</tr>';
                        }
                        if ($flag=='delete')
                        {
                            echo '<tr>';
                            echo '<td><input type="submit" name="btn" value="Delete" style="width:100px"></td>';
                            echo '</tr>';
                        }
                        echo '<tr>';
                        echo '<td><input type="submit" name="btn" value="Reset" style="width:100px"></td>';
                        echo '</tr>';

                        $contractreferencecategoryid_en = fnEncrypt('753621495');
                        $contractreferencedetailid_en = fnEncrypt($row['CONTRACTGUARANTORDETAILID']);
                        /* echo '<tr>';
                        echo '<td><a href="../data/contractaadhardetail_list.php?contractid='.fnEncrypt($contractid_de).'&contractreferencecategoryid='.$contractreferencecategoryid_en.'&contractreferencedetailid='.$contractreferencedetailid_en.'">Aadhar Detail</a></td>';
                        echo '</tr>'; */
                        if ($servicecontractorcategoryid_de == 324152658)
                        {
                            echo '<tr>';
                            echo '<td><a href="../data/contractfingerprintdetail_list.php?contractid='.fnEncrypt($contractid_de).'&contractreferencecategoryid='.$contractreferencecategoryid_en.'&contractreferencedetailid='.$contractreferencedetailid_en.'">Finger Print Detail</a></td>';
                            echo '</tr>';

                            echo '<tr>';
                            echo '<td><a href="../data/contractphotodetail_list.php?contractid='.fnEncrypt($contractid_de).'&contractreferencecategoryid='.$contractreferencecategoryid_en.'&contractreferencedetailid='.$contractreferencedetailid_en.'">Photo Detail</a></td>';
                            echo '</tr>';
                        }

                    echo '</table>';
                    echo '</form>';
                    echo '</section>';
                }
                else
                {
                    echo '<section>';
                    echo '<form method="post" action="../api_action/contractguarantordetail_action.php">';
                    echo '<table border="0" >';

                    if ($_SESSION['lng']=="English")
                    {
                        echo '<tr>';
                        echo '<td><label for="servicecontractorid">Service Contractor</label></td>';
                        echo '</tr>';
                    }
                    else
                    {
                        echo '<tr>';
                        echo '<td><label for="servicecontractorid">सेवा कंत्राटदार</label></td>';
                        echo '</tr>';
                    }
                    echo '<tr>';
                    echo '<td><input type="text" style="font-size:12pt;height:30px" name="servicecontractor" id="servicecontractor" style="width:300px"></td>';
                    echo '<td><label for="servicecontractorid">*</label></td>';
                    echo '</tr>';
                    echo '<tr>';
                    echo '<td><input readonly="readonly"  type="text" style="font-size:12pt;height:30px" name="servicecontractorid" id="servicecontractorid"></td>';
                    echo '</tr>';
                    if ($iscultivator == 1)
                    {
                        if ($_SESSION['lng']=="English")
                        {
                            echo '<tr>';
                            echo '<td><label for="fieldarea">Field Area</label></td>';
                            echo '</tr>';
                        }
                        else
                        {
                            echo '<tr>';
                            echo '<td><label for="fieldarea">क्षेत्र</label></td>';
                            echo '</tr>';
                        }

                        echo '<tr>';
                        echo '<td><input type="text" style="font-size:12pt;height:30px" name="fieldarea" id="fieldarea" style="width:300px" value="'.$row['FIELDAREA'].'"></td>';
                        echo '<td><label for="fieldarea">*</label></td>';
                        echo '</tr>';
                        echo '<tr>';
                            echo '<td><input type="text" style="font-size:12pt;height:30px" name="plantationarea" id="fieldarea" style="width:300px" value="'.$row['FIELDAREA'].'"></td>';
                            echo '<td><label for="plantationarea">*</label></td>';
                            echo '</tr>';
                            if ($_SESSION['lng']=="English")
                            {
                                echo '<tr>';
                                echo '<td><label for="plantationarea">Plantation Area</label></td>';
                                echo '</tr>';
                            }
                            else
                            {
                                echo '<tr>';
                                echo '<td><label for="plantationarea">लागवड क्षेत्र</label></td>';
                                echo '</tr>';
                            }

                            echo '<tr>';
                            echo '<td><input type="text" style="font-size:12pt;height:30px" name="plantationarea" id="plantationarea" style="width:300px" value="'.$row['PLANTATIONAREA'].'"></td>';
                            echo '<td><label for="plantationarea">*</label></td>';
                            echo '</tr>';
                    }
                    echo '<tr>';
                    echo '<tr>';
                    if ($_SESSION['lng']=="English")
                    {
                        echo '<tr>';
                        echo '<td><label for="fieldarea">Address</label></td>';
                        echo '</tr>';
                    }
                    else
                    {
                        echo '<tr>';
                        echo '<td><label for="fieldarea">पत्ता</label></td>';
                        echo '</tr>';
                    }
                    echo '<td><input type="text" style="font-size:12pt;height:30px" name="address" id="address"></td>';
                    echo '<td><label for="address">*</label></td>';
                    echo '</tr>';
                    echo '<td><input type="hidden" style="font-size:12pt;height:30px" name="contractid" id="contractid" style="width:300px" value ="'.$contractid_de.'"></td>';
                    echo '</tr>';
                    if ($servicecontractorcategoryid_de == 452168578)
                    {
                        echo '<input type="hidden" style="font-size:12pt;height:30px" name="iscultivator" id="iscultivator" style="width:300px" value ="'.$iscultivator.'">';
                    }
                    elseif ($servicecontractorcategoryid_de == 324152658)
                    {
                        echo '<input type="hidden" style="font-size:12pt;height:30px" name="iscultivator" id="iscultivator" style="width:300px" value ="'.$iscultivator.'">';
                    }
                    echo '<tr>';
                    echo '<td></td>';  
                    echo '</tr>';
                    echo '<tr>';
                    echo '<td><input type="submit" name="btn" value="Add" style="width:100px"</button>';
                    echo '</tr>';
                    echo '<tr>';
                    echo '<td><input type="submit" name="btn" value="Display" style="width:100px"</button>';
                    echo '</tr>';
                    echo '<tr>';
                    echo '<td><input type="submit" name="btn" value="Reset" style="width:100px"</button>';
                    echo '</tr>';
                    echo '</table>';
                    echo '</form>';
                    echo '</section>';
                }
            ?>
        </article>
        <footer>
        </footer>
    </body>
</html>