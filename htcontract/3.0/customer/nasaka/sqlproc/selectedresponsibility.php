<?php
    ...
    { */
        ...
        { */
            ...
            {
                if ($_SESSION["defyearcode"]==0)
                {
                    defaultyear();
                }
                else
                {
                    yearperiod($_SESSION["defyearcode"]);
                }
            }
    ...
    function defaultlegalentitycode()
    {
        ...
        if ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
        {
            ...
            $_SESSION["entityglobalgroupid"] = $row['LEGALENTITYCODE'];
            return true;
        }
        else
        {
    ...
    function defaultyear()
    {
        ...
        if ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
        {
            ...
            $_SESSION["finalreportperiodid"] =  $row['YEARPERIODCODE'];
    ...
    function yearperiod($yearcode)
    {
        $connection=swappdb_connection();
        if ($yearcode==1)
        {
            $query = "SELECT * FROM yearperiod d where defplantationyear=1";
        }
        elseif ($yearcode==2)
        {
            $query = "SELECT * FROM yearperiod d where defcrushingyear=1";
        }
        $result = oci_parse($connection, $query);
        $r = oci_execute($result);
        if (!$result)
