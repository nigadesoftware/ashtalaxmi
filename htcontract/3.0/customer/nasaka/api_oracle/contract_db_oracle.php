...
/* 					$result_11 = mysqli_query($this->connection, $query_11);
			...
			$query_12 = "select nvl(max(contractnumber),0)+1 as contractnumber from contract 
			where entityglobalgroupid=".$this->entityglobalgroupid.
			" and finalreportperiodid=".$this->finalreportperiodid.
			" and sugarfactoryid=".$this->sugarfactoryid.
			" and seasonid=".$this->seasonid.
			" and contractcategoryid=".$this->contractcategoryid;
	...
	public function display()
	{
		...
		if ($cond!='')
		{
			$query = "select c.contractid,t.name_eng,t.name_unicode,s.name_eng as seasonname_eng
			,s.name_unicode as seasonname_unicode,y.contractcategoryname_unicode,y.contractcategoryname_eng
			,contractnumber_prefixsuffix
			from contract c, season  s,servicecontractor t,contractcategory y 
			where c.active=1 and s.active=1 and c.seasonid=s.seasonid 
			and c.servicecontractorid=t.servicecontractorid 
			and c.contractcategoryid=y.contractcategoryid
			and ".$cond." order by contractnumber";
			//echo $query;
			$result = oci_parse($this->connection, $query);
			$r = oci_execute($result);
			return $result;
		...
		{
			$query = "select c.contractid,t.name_eng,t.name_unicode,s.name_eng as seasonname_eng,s.name_unicode as seasonname_unicode,y.contractcategoryname_unicode,y.contractcategoryname_eng  
			,contractnumber_prefixsuffix
			from contract c, season  s,servicecontractor t,contractcategory y  
			where c.active=1 and s.active=1 and t.active=1 
			and c.seasonid=s.seasonid 
			and c.servicecontractorid=t.servicecontractorid
			and c.contractcategoryid=y.contractcategoryid
