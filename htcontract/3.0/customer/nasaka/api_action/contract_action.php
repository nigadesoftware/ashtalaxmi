<?php
	...
	switch ($_POST['btn'])
	{
		case 'Add':
			...
			$contract1->address = $_POST["address"];
			$contract1->isadvance = $_POST["isadvance"];
			$contract1->contractorzonecode = $_POST["contractorzonecode"];
		...
		case 'Display':
			...
			if ($contract1->Get_invalidid()==0)
			{
				while ($row1 = oci_fetch_array($result1,OCI_ASSOC+OCI_RETURN_NULLS))
				{
					...
					{
						echo '<a href="../data/contract.php?contractid='.fnEncrypt($row1['CONTRACTID']).'&flag='.fnEncrypt('Display').'">'.$row1['NAME_ENG'].' - '.$row1['CONTRACTCATEGORYNAME_ENG'].'</br>';
					}
					else
					{
						echo '<a href="../data/contract.php?contractid='.fnEncrypt($row1['CONTRACTID']).'&flag='.fnEncrypt('Display').'">'.$row1['NAME_UNICODE'].' - '.$row1['CONTRACTCATEGORYNAME_UNICODE'].'</br>';
					}
				}	
			}
			else
