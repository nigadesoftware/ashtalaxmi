<?php
    ...
    if (isset($_GET['contractapproverdetailid']))
    {
        $contractapproverdetailid_de = fnDecrypt($_GET['contractapproverdetailid']);    
    }
...
<html>
    ...
    <body>
        <nav "w3-container">
            <ul class="navbar">
                ...
                <?php
                    ...
                    echo '<li><a class="navbar" href="../data/contractapproverdetail_list.php?contractid='.fnEncrypt($contractid_de).'&flag='.fnEncrypt('Display').'">Add/Display Contract Harvester Detail List</a></br>';
        ...
        <article class="w3-container">
            ...
            <?php
                $query = "select f.*,s.name_eng,s.name_unicode from contractapproverdetail f, employee s where f.active=1 and f.employeeid=s.employeeid and f.contractid = ".$contractid_de." and f.contractapproverdetailid=".$contractapproverdetailid_de;
                 //echo $query;
                ...
                if ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
                {
                    ...
                    //if ($flag_de == 'Display')
                    //{
                        echo '<form method="post" action="../api_action/contractapproverdetail_action.php">';
                    //}
                        echo '<table border="0" >';
                    ...
                    {
                        echo '<tr>';
                        echo '<td><label for="responsibilityid">Responsibility</label></td>';
                        echo '</tr>';
                    }
                    else
                    {
                        echo '<tr>';
                        echo '<td><label for="responsibilityid">जबाबदारी</label></td>';
                        echo '</tr>';
                    }
                    echo '<tr>';
                    $query = "select namedetailid,name_eng,name_unicode from namedetail n where n.active=1 and n.namecategoryid=843265874 order by name_eng";
                    $result1 = oci_parse($connection, $query); $r = oci_execute($result1);
                    echo '<tr>';
                    echo '<td><select name="responsibilityid" style="height:35px;font-size:14px;">';
                    if ($_SESSION['lng']=="English")
                    {
                    ...
                    while ($row1 = oci_fetch_array($result1,OCI_ASSOC+OCI_RETURN_NULLS))
                    {
                        if ($_SESSION['lng']=="English")
                        {
                            if ($row1['NAME_ENG'] == '-')
                            {
                                $name = $row1['NAME_UNICODE'];
                            }
                            else
                            {
                                $name = $row1['NAME_ENG'];
                            }
                            if ($row1['NAMEDETAILID'] == $row['RESPONSIBILITYID'])
                            {
                                echo '<option value="'.$row1['NAMEDETAILID'].'" Selected>'.$name.'</option>';
                            }
                            else
                            {
                                echo '<option value="'.$row1['NAMEDETAILID'].'">'.$name.'</option>';
                            }
                        }
                        else
                        {
                    ...
                    echo '<td><label for="responsibilityid">*</label></td>';
                    echo '</tr>';

                    if ($_SESSION['lng']=="English")
                    {
                        echo '<tr>';
                        echo '<td><label for="employeeid">Officer / Employee</label></td>';
                        echo '</tr>';
                    }
                    else
                    {
                        echo '<tr>';
                        echo '<td><label for="employeeid">अधिकारी / कामगार</label></td>';
                        echo '</tr>';
                    }
                    echo '<tr>';
                    if ($_SESSION['lng']=="English")
                    {
                        ...
                        echo '<td><input type="text" style="font-size:12pt;height:30px" name="employee" id="employee" style="width:300px" value="'.$name.'"></td>';
                    }
                    else
                    {
                        echo '<td><input type="text" style="font-size:12pt;height:30px" name="employee" id="employee" style="width:300px" value="'.$row['NAME_UNICODE'].'"></td>';
                    }
                    echo '<td><label for="employeeid">*</label></td>';
                    echo '<td><input type="hidden" style="font-size:12pt;height:30px" name="employeeid" id="employeeid"></td>';
                    echo '</tr>';
                ...
                else
                {
                    ...
                    echo '<tr>';
                    echo '<td><input type="text" style="font-size:12pt;height:30px" name="employee" id="employee" style="width:300px"></td>';
                    echo '<td><label for="employeeid">*</label></td>';
                    echo '<td><input type="hidden" style="font-size:12pt;height:30px" name="employeeid" id="employeeid"></td>';
                    echo '</tr>';
