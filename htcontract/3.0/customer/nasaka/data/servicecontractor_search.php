<?php
    ...
    if ($searchTerm == '**')
    {
        ...
        {
            $query = "select c.servicecontractorid,c.name_eng,c.name_unicode,c.refcode from servicecontractor c"; 
        }
    }
    else
    {
        ...
        {
            $query = "select c.servicecontractorid,c.name_eng,c.name_unicode,c.refcode from servicecontractor c where upper(c.name_eng) like upper('%".$searchTerm."%') or c.name_unicode like '".$searchTerm."%' or servicecontractorid=".$searchTerm." or upper(refcode)=upper('".$searchTerm."')"; 
        }
    }
    ...
    while ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
	{
        ...
        {
            $name = $row['NAME_ENG'].' ('.$row['REFCODE'].')';
        }
        else
        {
            $name = $row['NAME_UNICODE'].' ('.$row['REFCODE'].')';
        }
        $data[] = array( 
            'id' => $id
            ,'label' => $name
