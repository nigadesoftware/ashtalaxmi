...
{
	$fullname = str_replace('ठ','ञ',$fullname);
	$fullname = str_replace('ऱ','र',$fullname);
		...
		if ($i>0)
		{
			...
			else
			{
				...
				else
				{
					...
					elseif ($i==3)
					{
						$lastname1 .= mar2eng($output[$i]);
					}
					else
					{
						$lastname2 .= mar2eng($output[$i]);
					}
				}
			}
		}
	...
	elseif ($lastname1!='' and $lastname2=='')
	{
		$name = array(1=>$firstname,2=>$secondname,3=>$lastname.' '.$lastname1.'');
	}
	elseif ($lastname1=='' and $lastname2!='')
	{
		$name = array(1=>$firstname,2=>$secondname,3=>$lastname.' '.$lastname2.'');
	}
	elseif ($lastname1!='' and $lastname2!='')
	{
		$name = array(1=>$firstname,2=>$secondname,3=>$lastname.' '.$lastname1.' '.$lastname2.'');
	}
		return $name;
}
...
function convert()
{
	...
	require("../info/petrolpumproutine.php");
...
function mar2eng($marname)
{
	...
	for ($i=0;$i<strlen($marname);)
	{
		...
		//echo 'cur-'.$curakshar;
		//$marname=str_replace('(','',$marname);
		//$marname=str_replace(')','',$marname);
		if (in_array(substr($marname,$i,1),array(' ','(',')','-',',','/')))
		{
			$retstr=$retstr.substr($marname,$i,1);
			$i=$i+1;
		}
		else
		{
			$a=substr($marname,$i,3);
			switch (substr($marname,$i,3))
			{
				...
				case 'ं': case 'ँ':
					...
					{
						case 'प': case 'फ': case 'ब': case 'भ': case 'म':
							$retstr .='m';
							break;
						default:
							$retstr .='n';
...
function MarathitoIndianlanguage($name,$languagecode)
{
	$takshar='';
	for ($i=0;$i<strlen($name);)
	{
		if (in_array(substr($name,$i,1),array(' ','(',')','-',',','/','.')))
		{
			$takshar=$takshar.substr($name,$i,1);
			$i=$i+1;
		}
		else
		{
			$akshar=substr($name,$i,3);
			$nc = Hex2String(dechex(hexdec(String2Hex(substr($akshar,0,3)))+(512*$languagecode)));
			$takshar=$takshar.$nc;
			$i=$i+3;
		}
	}
...
function IndianlanguagetoMarathi($name,$languagecode)
{
	$takshar='';
	for ($i=0;$i<strlen($name);)
	{
...
function languagename($languagecode)
{
	...
	switch ($languagecode)
	{
		case 1:$languagename = 'Hindi/Marathi/etc.';
				break;
		case 3:$languagename = 'Gujarati';
				break;		
		case 6:$languagename = 'Telugu';
				break;	
		case 7:$languagename = 'Kannada';
				break;	
		case 0:$languagename = 'English';
				break;	
							
	}
	return $languagename;
}
function MarathitoEnglish($name)
{
	$ename = convertfullnamemar2eng(trim($name));
