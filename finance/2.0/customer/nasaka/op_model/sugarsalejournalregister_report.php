...
class sugarsalejournalregister extends swappreport
{	
    public $connection;
    public $liney;
    public $maxlines;
    ...
    public function __construct(&$connection,$maxlines,$isbig=0)
	{
        ...
        $this->pdf->SetSubject('Sugar Sale Journal Register');
        $this->pdf->SetKeywords('SUGSLJRNREG_000.MR');
        // set font
    ...
    function endreport()
    {
        ...
        $this->pdf->Output('SUGSLJRNREG_000.pdf', 'I');
    }
	function pageheader()
    {
        $this->liney = 15;
        ...
        $this->newrow(5);
        $this->textbox('साखर विक्री जर्नल रजिस्टर',200,10,'S','C',1,'siddhanta',13);
        $this->newrow(5);
    ...
    and r.vouchersubtypecode=14
        ...
        {  
            ...
            if ($row1['ISSUBLEDGERALLOWED']== 248805511)
            {
                ...
                {  
                    ...
                    $this->pdf->multicell(100,10,$row2['SUBLEDGERNAMEUNI'],0,'L',false,1,40,$this->liney,true,0,false,true,10);
                    $this->pdf->SetFont('SakalMarathiNormal922', '', 11, '', true);
                    if ($row2['CREDIT']!=0)
                    {
                        $this->pdf->multicell(35,10,$this->moneyFormatIndia($row2['CREDIT']),0,'R',false,1,165,$this->liney,true,0,false,true,10);
                        $totalcredit+=$row2['CREDIT'];
                    }
                    if ($row2['DEBIT']!=0)    
                    {
                        $this->pdf->multicell(35,10,$this->moneyFormatIndia($row2['DEBIT']),0,'R',false,1,130,$this->liney,true,0,false,true,10);
            ...
            else
            {
                ...
                if ($row1['CREDIT']!=0)
                {
                    $this->pdf->multicell(35,10,$this->moneyFormatIndia($row1['CREDIT']),0,'R',false,1,165,$this->liney,true,0,false,true,10);
                    $totalcredit+=$row1['CREDIT'];
                }
                if ($row1['DEBIT']!=0)
                {
                    $this->pdf->multicell(35,10,$this->moneyFormatIndia($row1['DEBIT']),0,'R',false,1,130,$this->liney,true,0,false,true,10);
        ...
        $this->pdf->SetFont('SakalMarathiNormal922', '', 11, '', true);
        $this->pdf->multicell(35,10,$this->moneyFormatIndia($totalcredit),0,'R',false,1,165,$this->liney,true,0,false,true,10);
        $this->pdf->multicell(35,10,$this->moneyFormatIndia($totaldebit),0,'R',false,1,130,$this->liney,true,0,false,true,10);
        $this->liney = $this->liney+5;
        $this->pdf->line(5,$this->liney,200,$this->liney);
