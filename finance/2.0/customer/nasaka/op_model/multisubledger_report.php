...
class multisubledger extends swappreport
{	
    public $fromdate;
    public $todate;
    public $accountcode;    
    ...
    function group()
    {
        $query1 = "select accountcode,subledgercode
        from accountsubledger 
        where referencecode='".$this->referencecode."'";
        $result1 = oci_parse($this->connection, $query1);
        $r = oci_execute($result1);
        while ($row1 = oci_fetch_array($result1,OCI_ASSOC+OCI_RETURN_NULLS))
        {
            $this->accountcode=$row1['ACCOUNTCODE'];
            $this->subledgercode=$row1['SUBLEDGERCODE'];
            $query2 = "select sum(openingbalance) openingbalance,sum(credit) credit,sum(debit) debit
                    from (
                    select 
                    subledgeropeningbalance(".$this->yearcode.
                    ",".$this->accountcode.
                    ",".$this->subledgercode.
                    ",'".$this->fromdate."') as openingbalance,0 credit,0 debit from dual
                    union all
                    select 0 opening
                    ...
                    having sum(openingbalance)<>0 or sum(credit)<>0 or sum(debit)<>0";
