...
class trialbalancegroup extends swappreport
{	
    public $connection;
    public $liney;
    public $maxlines;
	...
	function pageheader()
    {
        ...
        $this->pdf->multicell(60,10,'तेरीज पत्रक (ग्रुपवार)',0,'R',false,1,135,$this->liney,true,0,false,true,10);
...
function pagefooter($islastpage = false)
    ...
    function export()
    {
           ...
           while ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
           {
               ...
               if ($row['SEQUENCE']==7 or $row['SEQUENCE']==8 or $row['SEQUENCE']==9 or $row['SEQUENCE']==10) 
                    {
                        $accountnameuni=$arrow.'एकूण '.$row['ACCOUNTNAMEUNI'];
                        $accountcode=0;
                        //fputcsv($fp1, array($accountcode,$accountnameuni,0,0,0,0,0,0), $delimiter = ',', $enclosure = '"');
                    }
               else
                    {
                        $accountnameuni=$arrow.$row['ACCOUNTNAMEUNI'];
                        if ($row['ACCOUNTCODE']=='')
                        $accountcode=0;
                        else
               ...
               if ($row['SEQUENCE']==4 or $row['SEQUENCE']==7 or $row['SEQUENCE']==8 or $row['SEQUENCE']==9) 
               {
                    fputcsv($fp1, array($accountcode,$accountnameuni,$row['OPENINGBALANCE_CR'],$row['OPENINGBALANCE_DR'],$row['CREDIT'],$row['DEBIT'],$row['CLOSINGBALANCE_CR'],$row['CLOSINGBALANCE_DR']), $delimiter = ',', $enclosure = '"');
               }
               else
               {
                    fputcsv($fp1, array($accountcode,$accountnameuni,0,0,0,0,0,0), $delimiter = ',', $enclosure = '"');
            ...
            fputcsv($fp1, array(0,'एकूण एकंदर',$openingbalance_cr,$openingbalance_dr,$credit,$debit,$closingbalance_cr,$closingbalance_dr), $delimiter = ',', $enclosure = '"'); 
