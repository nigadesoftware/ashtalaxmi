...
<html>
    <head>
       ...
       <style type="text/css">
        ...
        <title>SubLedger Selection Parameter</title>
        <style type="text/css">
            body
            {
                background-color: #fff;
        ...
        <nav "w3-container">
            <ul >
                <?php
                    ...
                    echo '<a href="../mis/entitymenu.php">Entity Menu</br></a><a href="../op_view/monthlysubledger_selection.php?accountcode='.$accountcode_en.'&subledgercode='.$subledgercode_en.'">Monthly Ledger View</a><a style="color:#f48" class="navbar" href="../sqlproc/logout.php">Log Out</a>';
                ?>
            </ul>
        </nav>
            ...
            ?>
                ...
                <?php
                        ...
                        for($i=1;$i<=$maxdays;$i++)
                        {
                            ...
                            $openingbalance = $accountsubledger1->subopeningbalance($date1);
                            if ($i==1)
                            {
                               $openingbalancemonth=$openingbalance; 
                            }
                            $credit = $accountsubledger1->subbddebit($date1,$date1);
                            $debit = $accountsubledger1->subbdcredit($date1,$date1);
                            $credittotal=$credittotal+$credit;
                            $debittotal=$debittotal+$debit;
                            $closingbalance = $accountsubledger1->subclosingbalance($date1);
                            echo '<tr>';
                            echo '<td style="font-family:siddhanta;font-size:16px;"> <a style="color:green;" href="../op_view/subledger_view.php?accountcode='.$accountcode_en.'&subledgercode='.$subledgercode_en.'&monthcode='.$monthcode_en.'&date='.$date_en.'">'.$date.'</a></td>';
