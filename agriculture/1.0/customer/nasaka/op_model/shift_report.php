...
class shift extends swappreport
{
    public $slipdate;
	...
	function pageheader()
    {
        ...
        $this->textbox('दैनिक ऊस गाळप शिफ्टवाईज',195,10,'S','C',1,'siddhanta',15);
        $this->newrow(7);
        ...
        $this->textbox('मे.टन',$this->w,$this->x,'S','R',1,'siddhanta',11);
         ...
         $this->textbox('मे.टन',$this->w,$this->x,'S','R',1,'siddhanta',11);
         $this->hline(10,195,$this->liney+5,'C');
    ...
    { 
        ...
        ,sum(case when f.vehiclecategorycode = 3 then 1 else 0 end)  bull_cnt
        ,sum(case when f.vehiclecategorycode = 3 then t.netweight else 0 end)  bull_ton
        ,sum(case when f.vehiclecategorycode = 4 then 1 else 0 end)  jugad_cnt
        ,sum(case when f.vehiclecategorycode = 4 then t.netweight else 0 end)  jugad_ton
        ,sum(case when f.vehiclecategorycode = 1 then 1 else 0 end)  truck_cnt
        ,sum(case when f.vehiclecategorycode = 1 then t.netweight else 0 end)  truck_ton
        ,sum(case when f.vehiclecategorycode = 2 then 1 else 0 end)  tractor_cnt
        ,sum(case when f.vehiclecategorycode = 2 then t.netweight else 0 end) tractor_ton
        ,count(t.transactionnumber) tot_CNT
        ...
        while ($row = oci_fetch_array($result,oci_assoc+OCI_RETURN_NULLS))
        {
            ...
            $this->setfieldwidth(12);
            // Bullockcart
            $this->textbox($row['BULL_CNT'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
            $this->setfieldwidth(20);
            $this->textbox($this->numformat($row['BULL_TON'],3),$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
            //  Jugad
            $this->setfieldwidth(12);
            $this->textbox($row['JUGAD_CNT'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
            $this->setfieldwidth(20);
            $this->textbox($this->numformat($row['JUGAD_TON'],3),$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
            // Truck
            $this->setfieldwidth(12);
            $this->textbox($row['TRUCK_CNT'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
            $this->setfieldwidth(20);
            $this->textbox($this->numformat($row['TRUCK_TON'],3),$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
            // Tractor
            $this->setfieldwidth(12);
            $this->textbox($row['TRACTOR_CNT'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
            $this->setfieldwidth(20);
        ...
        // Day Total  start
        $query = "select sum(case when f.vehiclecategorycode = 3 then 1 else 0 end)  bull_cnt
        ,sum(case when f.vehiclecategorycode = 3 then t.netweight else 0 end)  bull_ton
        ,sum(case when f.vehiclecategorycode = 4 then 1 else 0 end)  jugad_cnt
        ,sum(case when f.vehiclecategorycode = 4 then t.netweight else 0 end)  jugad_ton
        ,sum(case when f.vehiclecategorycode = 1 then 1 else 0 end)  truck_cnt
