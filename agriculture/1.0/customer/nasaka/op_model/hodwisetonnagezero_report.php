...
class tonnagesmry extends reportbox
{
    ...
    function pageheader()
    {
        ...
        $this->textbox('Agriculture Contractor wise  Zero Tonnage  Report',$this->w,$this->x,'S','C',1,'verdana',11,'','','','B');
        $this->newrow();
    ...
    function group()
    {
        ...
        $group_query_1 =
         "  select c.hodcode,h.hodnameeng,c.contractorcode,c.contractornameeng,0 wt
         ,row_number() over (partition by c.hodcode order by t.CONTRACTORCODE) srnumber    
         from HT_CONTRACT_DATA t,contractor c
        ,hodcode h
        where t.CONTRACTORCODE=c.contractorcode
        and c.hodcode=h.hodcode
        and seasoncode = ".$_SESSION['yearperiodcode'] ." and 
        t.contractorcode not in
        (select distinct(tt.CONTRACTORCODE) from HT_TONNAGE tt where tt.SEASONCODE=".$_SESSION['yearperiodcode'] ." )
        order by c.hodcode,t.contractorcode
           ";   
    ...
    function groupheader_1(&$group_row_1)
    {
        ...
        $this->textbox('HOD Name: '.$group_row_1['HODCODE'].' - '.$group_row_1['HODNAMEENG'],$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','B');  
