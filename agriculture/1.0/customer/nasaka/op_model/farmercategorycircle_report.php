...
class farmercategorycirclecrushing extends reportbox
{
    public $fromdate;
    public $todate;
    ...
    public function __construct(&$connection,$maxlines,$languagecode=1,$rowheight=7)
	{
        ...
        $this->pdf->SetSubject('Farmer Category wise Circlewise');
        $this->pdf->SetKeywords('FARCIRCRUSH_000.MR');
        // set font
	...
	function pageheader()
    {
        ...
        $this->textbox('सभासदप्रकारनिहाय गटनिहाय ऊस गाळप '.$_SESSION['yearperiodcode'],180,10,'S','C',1,'siddhanta',13);
        ...
        $this->textbox('सभासद प्रकार',$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(30);
        $this->textbox('गट',$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(25);
        $this->textbox('गाव संख्या',$this->w,$this->x,'S','R',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(25);
        $this->textbox('ऊ.उत्पा.संख्या',$this->w,$this->x,'S','R',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(25);
        $this->textbox('एकूण टनेज',$this->w,$this->x,'S','R',1,'siddhanta',10,'','','','B');
        $this->newrow(7);
    ...
    function group()
    {
        ...
        $group_query_1 = "select farmercategorycode,circlecode,farmercategorynameuni,circlenameuni
        ,villagecode,sum(farmercnt) farmercnt,sum(netweight) netweight
        from (
        select farmercategorycode,farmercategorynameuni,circlecode,circlenameuni
        ,villagecode,farmercode
        ,1 farmercnt
        ,sum(netweight) netweight
        from (    
            select y.farmercategorycode,y.farmercategorynameuni,v.circlecode,v.circlenameuni,f.villagecode,f.farmercode
            ,t.netweight
            from weightslip t,fieldslip f
            ,circle v,village c,farmer p,farmercategory y
            where
            t.seasoncode=f.seasoncode 
            and t.fieldslipnumber=f.fieldslipnumber
            and f.farmercategorycode=y.farmercategorycode
            and f.villagecode=c.villagecode
            and f.farmercode=p.farmercode
            and c.circlecode=c.circlecode
            and nvl(t.netweight,0)>0
            and {$cond}
            )
        group by farmercategorycode,farmercategorynameuni,circlecode,circlenameuni,villagecode,farmercode)
        group by farmercategorycode,farmercategorynameuni,circlecode,circlenameuni
        ,villagecode
        order by farmercategorycode,farmercategorynameuni,circlecode,circlenameuni
        ,villagecode
";
    ...
    function groupheader_1(&$group_row_1)
    {
        ...
        $this->farmercategory['MEMBER_TON']=0;
        $this->farmercategory['NONMEMBER_TON']=0;
        $this->farmercategory['GATECANE_TON']=0;
        $this->setfieldwidth(30,10);
    ...
    function detail_1(&$group_row_1)
    {
        ...
        $this->farmercategory['MEMBER_TON']=$this->farmercategory['MEMBER_TON']+$group_row_1['MEMBER_TON'];
        $this->summary['MEMBER_TON']=$this->summary['MEMBER_TON']+$group_row_1['MEMBER_TON'];
        $this->farmercategory['NONMEMBER_TON']=$this->farmercategory['NONMEMBER_TON']+$group_row_1['NONMEMBER_TON'];
        $this->summary['NONMEMBER_TON']=$this->summary['NONMEMBER_TON']+$group_row_1['NONMEMBER_TON'];
        $this->farmercategory['GATECANE_TON']=$this->farmercategory['GATECANE_TON']+$group_row_1['GATECANE_TON'];
        $this->summary['GATECANE_TON']=$this->summary['GATECANE_TON']+$group_row_1['GATECANE_TON'];
        
        $this->farmercategory['MEMBER_CNT']=$this->farmercategory['MEMBER_CNT']+$group_row_1['MEMBER_CNT'];
        $this->summary['MEMBER_CNT']=$this->summary['MEMBER_CNT']+$group_row_1['MEMBER_CNT'];
        $this->farmercategory['NONMEMBER_CNT']=$this->farmercategory['NONMEMBER_CNT']+$group_row_1['NONMEMBER_CNT'];
        $this->summary['NONMEMBER_CNT']=$this->summary['NONMEMBER_CNT']+$group_row_1['NONMEMBER_CNT'];
        $this->farmercategory['GATECANE_CNT']=$this->farmercategory['GATECANE_CNT']+$group_row_1['GATECANE_CNT'];
    ...
    {     
        ...
        $this->setfieldwidth(25);
        $this->textbox($this->numformat($this->farmercategory['MEMBER_TON'],3),$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
        $this->setfieldwidth(25);
        $this->textbox($this->numformat($this->farmercategory['NONMEMBER_TON'],3),$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
        $this->setfieldwidth(25);
        $this->textbox($this->numformat($this->farmercategory['GATECANE_TON'],3),$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
        $this->setfieldwidth(25);
        $this->textbox($this->numformat($this->farmercategory['MEMBER_TON']+$this->farmercategory['NONMEMBER_TON']+$this->farmercategory['GATECANE_TON'],3),$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
        $this->hline(10,200,$this->liney,'C');  
        $this->newrow(4);
        $this->setfieldwidth(30,10);
        $this->setfieldwidth(30);
        //$this->textbox('गट एकूण',$this->w,$this->x,'S','L',1,'siddhanta',11,'','','','B');
        $this->setfieldwidth(25);
        $this->textbox($this->farmercategory['MEMBER_CNT'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
        $this->setfieldwidth(25);
        $this->textbox($this->farmercategory['NONMEMBER_CNT'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
        $this->setfieldwidth(25);
        $this->textbox($this->farmercategory['GATECANE_CNT'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
        $this->setfieldwidth(25);
        $this->textbox($this->farmercategory['MEMBER_CNT']+$this->farmercategory['NONMEMBER_CNT']+$this->farmercategory['GATECANE_CNT'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10);
        //$this->hline(10,200,$this->liney,'C');  
        $this->newrow();
        $this->hline(10,200,$this->liney-2,'C');  
