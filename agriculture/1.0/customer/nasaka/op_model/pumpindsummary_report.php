...
class pumpdatesummary extends reportbox
{
	...
	function pageheader()
    {
        ...
        $this->newrow(7);
        $this->textbox('पंपनिहाय डिझेल समरी',180,10,'S','C',1,'siddhanta',12);
        $this->newrow(7);
        ...
        $this->textbox('खरेदी दर',$this->w,$this->x,'S','R',1,'siddhanta',11);
        $this->setfieldwidth(35);
        $this->textbox('ख.रक्कम',$this->w,$this->x,'S','R',1,'siddhanta',11);
        $this->setfieldwidth(35);
        $this->textbox('वि.रक्कम',$this->w,$this->x,'S','R',1,'siddhanta',11); 
        $this->setfieldwidth(35);
        $this->textbox('फरक रक्कम',$this->w,$this->x,'S','R',1,'siddhanta',11); 
        $this->newrow();
    ...
    function group()
    {
        ...
        t.pumpcode,pumpnameuni,contractornameuni nname,contractorcode
        ,sum(quantity) quantity,max(purchaserate) purchaserate
        ...
        ,contractornameuni,contractorcode
        order by t.pumpcode,pumpnameuni,contractorcode";
        }
        else
        {
            $group_query_1 = "            
            select t.pumpcode,pumpnameuni,contractorvehiclecode
            ,contractornameuni nname,vehiclenumber
            ,sum(quantity) quantity,max(purchaserate) purchaserate
            ,sum(purchaseamount) purchaseamount
            ,max(salerate) salerate,sum(saleamount) saleamount
            ,sum(purchaseamount)-sum(saleamount) diffamount
            from (
            select pumpcode,contractorcategorycode,0 contractorcode,vehiclecode,null subcontractornameuni
            ,contractorcategorynameuni,contractornameuni,contractorvehiclecode
            ,vehiclenumber,transactiondate,quantity,purchaserate
            ,purchaseamount,salerate,saleamount,slipnumber
            from  (
            select 0 contractorcategorycode,t.contractorvehiclecode,d.vehiclenumber vehiclecode
            ,'' subcontractornameuni ,'कारखाना स्वःताचे' contractorcategorynameuni,d.contractornameuni contractornameuni
            ,d.vehiclenumber,t.transactiondate,t.quantity,t.purchaserate
            ,t.purchaseamount,t.salerate,t.saleamount,t.pumpcode,t.slipnumber
            from PUMPDIESELTRANSACTION t,othercontractor d
            where  t.contractorvehiclecode=d.contractorcode
            and {$cond}))t,pump p
            where t.pumpcode=p.pumpcode
            group by t.pumpcode,pumpnameuni ,contractorvehiclecode,contractornameuni,vehiclenumber
            order by t.pumpcode,pumpnameuni,contractorvehiclecode
    ...
    function groupheader_1(&$group_row_1)
    {
        ...
        $this->textbox($group_row_1['PUMPNAMEUNI'],$this->w,$this->x,'S','L',1,'siddhanta',12,'','','B');
        $this->newrow(5);
        $this->hline(10,200,$this->liney,'D'); 
    }
    ...
    function detail_1(&$group_row_1)
    {
       ...
       $ht=$this->height($group_row_1['NNAME'],40);
        ...
        $this->textbox($group_row_1['NNAME'],$this->w,$this->x,'N','L',1,'siddhanta',10);
        $this->setfieldwidth(20);
        $this->textbox($group_row_1['QUANTITY'],$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',11);
        $this->setfieldwidth(20);
        $this->textbox($group_row_1['PURCHASERATE'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',11);
        $this->setfieldwidth(35);
        $this->textbox($this->numformat($group_row_1['PURCHASEAMOUNT'],2),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',11);
        $this->setfieldwidth(35);
        $this->textbox($this->numformat($group_row_1['SALEAMOUNT'],2),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',11); 
        $this->setfieldwidth(35);
        $this->textbox($this->numformat(($group_row_1['PURCHASEAMOUNT']-$group_row_1['SALEAMOUNT']),2),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',11); 
        $this->newrow($ht);
    ...
    {     
        ...
        $this->setfieldwidth(20);
        $this->setfieldwidth(35);
        $this->textbox($this->numformat($this->pumpsummary['PURCHASEAMOUNT'],2),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',11); 
        $this->setfieldwidth(35);
    ...
    function reportfooter()
    {
        ...
        $this->textbox($this->numformat($this->summary['PURCHASEAMOUNT'],2),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',11); 
        $this->setfieldwidth(30);
