...
class centreplantationsummary extends reportbox
{
    public $centrecode;
    public $fromdate;
    public $todate;
    ...
    function group()
    {
        ...
        if ($this->fromdate!='' and $this->todate!='')
        {
            ...
            ,0 as total
            from 
            (select vv.centrecode,cc.centrenameuni,pp.farmercode 
            from plantationheader pp,village vv,centre cc
            where pp.villagecode=vv.villagecode 
            and vv.centrecode=cc.centrecode
            and seasoncode=".$_SESSION['yearperiodcode']."
            and pp.plantationdate>='".$this->fromdate."'
            and pp.plantationdate<='".$this->todate."'
            group by vv.centrecode,cc.centrenameuni,pp.farmercode)t 
            group by t.centrecode,t.centrenameuni
            )ttt,centre vvv,division d
            where ttt.centrecode=vvv.centrecode
            and vvv.divisioncode=d.divisioncode
            group by d.divisioncode,d.divisionnameuni,vvv.centrecode,vvv.centrenameuni
            order by d.divisioncode,vvv.centrecode
            ";
        }
        else
        {
