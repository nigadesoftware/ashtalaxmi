...
class tonnage extends reportbox
{
	...
	function pageheader()
    {
        ...
        $this->newrow(7);
        $this->textbox('ऊस आलेल्या शेतकरी टनेज लिस्ट',180,10,'S','C',1,'siddhanta',12);
        $this->newrow(7);
    ...
    function group()
    {
        ...
        $group_query_1 = "  select  m.centrecode,m.centrenameuni
        ,m.villagecode ,m.villagenameuni
        ,m.farmercode,m.farmernameuni
        ,m.plotnumber
        ,(select sum(h.area) from plantationheader h where h.seasoncode=".$_SESSION['yearperiodcode']." and h.plotnumber=m.plotnumber)area
        ,wt from
        (select 
        c.centrecode,c.centrenameuni
        ,f.villagecode ,v.villagenameuni
        , f.farmercode,ff.farmernameuni
        ,f.plotnumber
        ,sum(w.netweight)wt
        from weightslip w,fieldslip f
        ,village v,centre c,farmer ff
         where {$cond} and
        w.seasoncode=f.seasoncode
        and w.fieldslipnumber=f.fieldslipnumber
        and f.villagecode=v.villagecode
        and v.centrecode=c.centrecode
        and f.farmercode=ff.farmercode
        and w.netweight>0
        group by f.farmercode,f.plotnumber,f.villagecode
        ,v.villagenameuni,c.centrecode
        ,c.centrenameuni,ff.farmernameuni
        order by c.centrecode,f.villagecode,f.farmercode
        )m";
