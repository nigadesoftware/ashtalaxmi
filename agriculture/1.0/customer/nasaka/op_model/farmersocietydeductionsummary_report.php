...
class farmersocietydeductionsummary extends reportbox
{
    ...
    function pageheader()
    {
        ...
        $this->textbox('ऊस पेमेंट सोसायटी कपात समरी',$this->w,$this->x,'S','C',1,'siddhanta',11,'','','','B');
        $this->newrow();
        $this->setfieldwidth(190,10);
    ...
    function group()
    {
        ...
        $group_query_1 ="select societybranchcode
        ,societybranchnameuni
        ,societybanknameuni
        ,branchcode
        ,bankbranchnameuni
        ,sum(deductionamount) as deductionamount
        from (select societybranchcode
        ,societybranchnameuni
        ,societybanknameuni
        ,branchcode
        ,bankbranchnameuni
        ...
        group by societybranchcode
        ,societybranchnameuni
        ,societybanknameuni
        ,branchcode
        ,bankbranchnameuni 
        union all
        select societybranchcode
        ,societybranchnameuni
        ,societybanknameuni
        ,branchcode
        ,bankbranchnameuni
        ,sum(deductionamount) as deductionamount
        from (select 
        b.bankbranchcode societybranchcode
        ,d.branchcode
        ,d.deductioncode
        ,d.dedseasonyear
        ,m.deductionnameuni
        ...
        ,v.villagenameuni
        ,c.banknameuni
        ,b.bankbranchnameuni||' बँक' bankbranchnameuni
        ,c.banknameuni societybanknameuni
        ,b.bankbranchnameuni societybranchnameuni
        ,d.deductionamount deductionamount
        from farmerbillheader t, farmerbilldeductiondetail d
        ,farmer f, village v
        ,billperiodheader h, deduction m
    ...
    function groupheader_1(&$group_row_1)
    {
        ...
        $this->textbox($group_row_1['SOCIETYBANKNAMEUNI'].' '.$group_row_1['SOCIETYBRANCHNAMEUNI'],$this->w,$this->x,'S','L',1,'siddhanta',11,'','','','');
        $this->srno=0;
        $this->newrow();
    }
    ...
    function detail_1(&$group_row_1)
    {
        ...
        $this->setfieldwidth(10,30);
        $this->textbox(++$this->srno,$this->w,$this->x,'N','L',1,'SakalMarathiNormal922',10,'','','','');
        $this->setfieldwidth(80);
        $this->textbox($group_row_1['BANKBRANCHNAMEUNI'],$this->w,$this->x,'S','L',1,'SakalMarathiNormal922',10,'','','','');
        $this->setfieldwidth(35);
        ...
        $this->summary['DEDUCTIONAMOUNT']=$this->summary['DEDUCTIONAMOUNT']+$group_row_1['DEDUCTIONAMOUNT'];
    }
    ...
    { 
        $this->setfieldwidth(10,30);
        $this->setfieldwidth(80);
        $this->textbox('शाखा एकूण',$this->w,$this->x,'S','L',1,'SakalMarathiNormal922',10,'','','','');
        $this->setfieldwidth(35);
        $this->textbox($this->branchsummary['DEDUCTIONAMOUNT'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',11,'','','','');    
        $this->newrow();
    }
    ...
    function reportfooter()
    {
        ...
        $this->setfieldwidth(10,30);
        $this->setfieldwidth(80);
        $this->textbox('एकूण एकंदर',$this->w,$this->x,'S','L',1,'SakalMarathiNormal922',10,'','','','B');
        $this->setfieldwidth(35);
        $this->textbox($this->summary['DEDUCTIONAMOUNT'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',11,'','','','');    
        if ($this->isnewpage(15))
        {
            $this->newrow();
            $this->hline(10,195,$this->liney-2,'C'); 
