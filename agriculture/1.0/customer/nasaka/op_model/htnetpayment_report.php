...
class htnetpayment extends reportbox
{
    public $seasoncode;
    ...
    function pageheader()
    {
        ...
        $this->textbox('तोडणी वहातूक पेमेंट निव्व्ळ देय यादी',$this->w,$this->x,'S','C',1,'siddhanta',11,'','','','B');
        $this->newrow();
        $this->setfieldwidth(190,10);
        $this->textbox($this->msubtitle,$this->w,$this->x,'S','C',1,'siddhanta',10,'','','','');
        $this->newrow();
        $this->setfieldwidth(195,10);
        $this->textbox('बँक : '.$this->bankname.'  ब्रांच : '.$this->bankbranchname.'  '.$this->contractorcategoryname,$this->w,$this->x,'S','L',1,'siddhanta',12,'','','','B');
        $this->newrow();
        ...
        $this->textbox('तो व कंत्राटदार नाव',$this->w,$this->x,'S','L',1,'siddhanta',11,'','','','B');
    ...
    function group()
    {
        if ($this->flagcode == 1)
        {
            $cond = "h.servicetrhrcategorycode in (1)";
        }
        else
        {
            $cond = "h.servicetrhrcategorycode not in (1)";
        }
        if ($this->bankbranchcode!=0 or $this->bankbranchcode!='')
        {
            ...
            ,f.contractorcode
            ,k.contractornameuni
            ,f.address villagenameuni
            ,sum(t.netamount) netamount
            ,t.accountnumber
            ,b.bankbranchnameuni
            from htbillheader t,contractor f
            ,billperiodheader h,bankbranch b, bank c,Bankcategory r
            ,contractorwithadhikarpatra k
            ...
            and t.seasoncode=k.seasoncode and t.contractorcode=k.contractorcode
        ...
        elseif ($this->bankcode!=0 or $this->bankcode!='')
        {
            $group_query_1 ="select 
            c.bankcode
            ,t.bankbranchcode
        ...
        elseif (($this->bankcode==0 and $this->bankbranchcode==0) or ($this->bankcode=='' and $this->bankbranchcode==''))
        {
            $group_query_1 ="select 
            c.bankcode
            ,t.bankbranchcode
