...
class htbillcontractor extends swappreport
{
    public $billperiodtransnumber;
    public $msubtitle;
    public $billsummary;
    public $slipsummary;
    public $contractorbillsummary;
    public $servicetrhrcategorycode;
    public $billcategorycode;
    public $fieldslipcount;
    ...
    function pageheader()
    {
        //$this->pdf->Image("../img/kadwawatermark.png", 60, 40, 70, 70, '', '', '', false, 300, '', false, false, 0);
    }
    ...
    function startreport()
    {
        ...
        $query ="select t.servicetrhrcategorycode,t.billcategorycode from billperiodheader t where t.billperiodtransnumber={$this->billperiodtransnumber}";
        ...
        {
            $this->servicetrhrcategorycode = $group_row_1['SERVICETRHRCATEGORYCODE'];
    ...
    function group()
    {
        ...
        $this->slipsummary['SLIPSUMMARY']=0;
        if ($this->servicetrhrcategorycode != 6)
        {
            $group_query_1 ="select transactionnumber,f.contractorcode
            ,t.subcontractorcode,t.vehiclecode,v.vehiclenumber
            ,to_char(h.paymentdate,'dd/MM/yyyy') paymentdate
            ,f.contractornameuni
            ,t.accountnumber,b.bankbranchnameuni,bk.banknameuni,bk.shortname
            ,h.servicetrhrcategorycode,sc.servicetrhrcatnameuni
            ,t.billperiodtransnumber
            ,sum(t.incentiveamount) incentiveamount,sum(t.grossamount) grossamount
            ,sum(t.grossdeduction) grossdeduction
            ,sum(t.depositamount) depositamount,sum(t.commissionamount) commissionamount
            ,sum(t.netgrossamount) netgrossamount,sum(t.netamount) netamount
            ,sum(t.hrtonnage) hrtonnage,sum(t.trtonnage) trtonnage,sum(t.hrtrtonnage) hrtrtonnage
            ,sum(t.businessamount) businessamount,sum(t.dieselratedifferenceamount) dieselratedifferenceamount
            ...
            ,vehicle v,servicetrhrcategory sc
            ...
            group by transactionnumber,f.contractorcode,t.subcontractorcode,t.vehiclecode,v.vehiclenumber
            ,to_char(h.paymentdate,'dd/MM/yyyy')
            ,f.contractornameuni
            ,t.accountnumber,b.bankbranchnameuni,bk.banknameuni,bk.shortname
            ,h.servicetrhrcategorycode,sc.servicetrhrcatnameuni
            ,t.billperiodtransnumber,t.billnumber
            order by billnumber";
        }
        elseif ($this->servicetrhrcategorycode == 6)
        {
            $group_query_1 ="select f.contractorcode
            ,null subcontractorcode,null vehiclecode,null vehiclenumber
            ,to_char(h.paymentdate,'dd/MM/yyyy') paymentdate
            ,f.contractornameuni
            ,t.accountnumber,b.bankbranchnameuni,bk.banknameuni,bk.shortname
            ,h.servicetrhrcategorycode,sc.servicetrhrcatnameuni
            ...
            and s.harvestersubcategorycode=hs.harvestersubcategorycode(+)
            and t.billperiodtransnumber=".$this->billperiodtransnumber." 
            group by f.contractorcode
            ,to_char(h.paymentdate,'dd/MM/yyyy')
            ,f.contractornameuni
            ,t.accountnumber,b.bankbranchnameuni,bk.banknameuni,bk.shortname
            ,h.servicetrhrcategorycode,sc.servicetrhrcatnameuni
            ,t.billperiodtransnumber
            order by f.contractorcode";
        }
        ...
        while ($group_row_1 = oci_fetch_array($group_result_1,OCI_ASSOC+OCI_RETURN_NULLS))
        {
            ...
            $this->detail_0($group_row_1['CONTRACTORCODE']);
            $this->detail_1($group_row_1);
    ...
    function groupheader_1(&$group_row_1)
    {
        ...
        $this->textbox('कंत्राटदार : '.$group_row_1['CONTRACTORCODE'].' '.$group_row_1['CONTRACTORNAMEUNI'].' '.$group_row_1['VEHICLENUMBER'],$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','B');
        $this->newrow();
        ...
        $this->textbox('तो.टनेज',$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(20);
        $this->textbox('व.टनेज',$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(20);
        $this->textbox('तो.व.टनेज',$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(20);
        $this->textbox('धंदा',$this->w,$this->x,'S','R',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(20);
        $this->textbox('डिपाॅझीट',$this->w,$this->x,'S','R',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(20);
        $this->textbox('कमिशन',$this->w,$this->x,'S','R',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(20);
        $this->textbox('ए.रक्कम',$this->w,$this->x,'S','R',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(20);
        $this->textbox('क.रक्कम',$this->w,$this->x,'S','R',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(25);
        $this->textbox('देय रक्कम',$this->w,$this->x,'S','R',1,'siddhanta',10,'','','','B');
        $this->newrow(7);
        $this->hline(10,195,$this->liney,'D');
        $this->detail_1($group_row_1); */
    ...
    function detail_0($contractorcode)
    { 
        $detail_query_1 = "select h.servicehrtrcategorycode,h.transactionnumber,bh.billcategorycode
        ,ty.tyregadicode,ty.tyregadinumber,ty.gadiwannameuni
        ,sum(d.tramount) tramount,sum(d.hramount) hramount,sum(d.sliptonnage) sliptonnage
        ,count(t.fieldslipnumber) fieldslipcount
        from htbillheader h,htbillslipdetail d,village v,farmer f,fieldslip t
        ,weightslip w
        ,billperiodheader bh,
        tyregadi ty
        ...
        and h.billperiodtransnumber=bh.billperiodtransnumber
        and t.seasoncode=ty.seasoncode(+)
        and t.tyregadicode=ty.tyregadicode(+)
        and h.billperiodtransnumber=".$this->billperiodtransnumber." 
        and h.contractorcode=".$contractorcode." 
        group by h.servicehrtrcategorycode,h.transactionnumber
        ,bh.billcategorycode
        ,ty.tyregadicode,ty.tyregadinumber,ty.gadiwannameuni
        order by h.servicehrtrcategorycode,h.transactionnumber
        ,bh.billcategorycode
        ,ty.tyregadicode,ty.tyregadinumber,ty.gadiwannameuni
        ";
        ...
        while ($detail_row_1 = oci_fetch_array($detail_result_1,OCI_ASSOC+OCI_RETURN_NULLS))
        {
            ...
            if ($detail_row_1['BILLCATEGORYCODE']==1)
            {
            $this->setfieldwidth(35,10);
            $this->textbox($detail_row_1['TYREGADINUMBER'].' '.$detail_row_1['GADIWANNAMEUNI'],$this->w,$this->x,'S','L',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(30);
            $this->textbox('शेतकरी',$this->w,$this->x,'S','L',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(10);
            $this->textbox('गाव',$this->w,$this->x,'S','L',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(15);
            $this->textbox('ए.स्लिप ',$this->w,$this->x,'S','R',1,'siddhanta',8,'','','','');

            $this->setfieldwidth(10,95);
            if ($detail_row_1['SERVICEHRTRCATEGORYCODE']!=2)
            $this->textbox('अंतर',$this->w,$this->x,'N','R',1,'siddhanta',10,'','','','');
            $this->setfieldwidth(20);
            $this->textbox('टनेज',$this->w,$this->x,'N','R',1,'siddhanta',10,'','','','');
            $this->setfieldwidth(15);
            $this->textbox('वाह.दर',$this->w,$this->x,'N','R',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(15);
            $this->textbox('वाह.रक्कम',$this->w,$this->x,'N','R',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(15);
            $this->textbox('तोड.दर',$this->w,$this->x,'N','R',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(15);
            $this->textbox('तोड.रक्कम',$this->w,$this->x,'N','R',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(15);
            $this->textbox('एकूण',$this->w,$this->x,'N','R',1,'siddhanta',8,'','','','');
            $this->newrow(5);
            $this->detail_00($contractorcode,$detail_row_1['TYREGADICODE']);
            $this->setfieldwidth(30,10);
            //$this->textbox('एकूण '.$detail_row_1['TYREGADINUMBER'].' '.$detail_row_1['GADIWANNAMEUNI'],$this->w,$this->x,'S','L',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(45);
            $this->textbox('एकूण '.$detail_row_1['TYREGADINUMBER'].' '.$detail_row_1['GADIWANNAMEUNI'],$this->w,$this->x,'S','L',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(15);
            $this->fieldslipcount=$this->fieldslipcount+$detail_row_1['FIELDSLIPCOUNT'];
            $this->textbox($detail_row_1['FIELDSLIPCOUNT'],$this->w,$this->x,'S','R',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(10,95);
            if ($detail_row_1['SERVICEHRTRCATEGORYCODE']!=2)
            ...
            $this->setfieldwidth(15);
            //$this->textbox($this->nvl($detail_row_1['TRRATE']),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',8,'','','','');
            $this->setfieldwidth(15);
            $this->textbox($this->nvl($detail_row_1['TRAMOUNT']),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',8,'','','','');
            $this->setfieldwidth(15);
            //$this->textbox($this->nvl($detail_row_1['HRRATE']),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',8,'','','','');
            $this->setfieldwidth(15);
            $this->textbox($this->nvl($detail_row_1['HRAMOUNT']),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',8,'','','','');
            $this->setfieldwidth(15);
            $this->textbox($this->nvl($detail_row_1['HRAMOUNT']+$detail_row_1['TRAMOUNT']),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',8,'','','','');
            $this->newrow(5);
            }
            ...
            $this->slipsummary['SLIPSUMMARY'] =  $this->slipsummary['SLIPSUMMARY']+$detail_row_1['FFIELDSLIPNUMBER'];
        }
            $this->hline(85,195,$this->liney,'D');
            if ($detail_row_1['BILLCATEGORYCODE']==1)
            {
            $this->setfieldwidth(10,65);
            $this->textbox(number_format_indian($this->slipsummary['SLIPSUMMARY'],3),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',10,'','','','');
            $this->setfieldwidth(20);
    ...
    function detail_00($contractorcode,$tyregadicode)
    { 
        $detail_query_1 = "select h.servicehrtrcategorycode,h.transactionnumber,bh.billcategorycode
        ,ty.tyregadicode,ty.tyregadinumber,ty.gadiwannameuni
        ,v.villagecode,v.villagenameuni
        ,f.farmercode,f.farmernameuni,d.distance
        ,d.trrate,d.hrrate,sum(d.tramount) tramount,sum(d.hramount) hramount,sum(d.sliptonnage) sliptonnage
        ,count(t.fieldslipnumber)ffieldslipnumber
        from htbillheader h,htbillslipdetail d,village v,farmer f,fieldslip t
        ,weightslip w
        ,billperiodheader bh,
        tyregadi ty
        ...
        and h.billperiodtransnumber=".$this->billperiodtransnumber." 
        and h.contractorcode=".$contractorcode." 
        and ty.tyregadicode=".$tyregadicode."
        group by h.servicehrtrcategorycode,h.transactionnumber,bh.billcategorycode,ty.tyregadicode,ty.tyregadinumber,ty.gadiwannameuni,v.villagecode,v.villagenameuni,f.farmercode,f.farmernameuni,d.distance,d.trrate,d.hrrate
        order by h.servicehrtrcategorycode,h.transactionnumber,bh.billcategorycode,ty.tyregadicode,ty.tyregadinumber,ty.gadiwannameuni,v.villagecode,v.villagenameuni,f.farmercode,f.farmernameuni,d.distance,d.trrate,d.hrrate";
        $detail_result_1 = oci_parse($this->connection, $detail_query_1);
        $r = oci_execute($detail_result_1);
        $islastodd=0;
        $this->billsummary['TRAMOUNT']=0;
        ...
        while ($detail_row_1 = oci_fetch_array($detail_result_1,OCI_ASSOC+OCI_RETURN_NULLS))
        {
            ...
            //$this->textbox($detail_row_1['TYREGADINUMBER'].' '.$detail_row_1['GADIWANNAMEUNI'],$this->w,$this->x,'S','L',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(30);
            $this->textbox($detail_row_1['FARMERNAMEUNI'],$this->w,$this->x,'S','L',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(20);
            $this->textbox($detail_row_1['VILLAGENAMEUNI'],$this->w,$this->x,'S','L',1,'siddhanta',8,'','','','');
            $this->setfieldwidth(10);
            $this->textbox($detail_row_1['FFIELDSLIPNUMBER'],$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','');
            ...
            $this->hline(85,200,$this->liney,'D');
            /* if ($detail_row_1['BILLCATEGORYCODE']==1)
            {
            $this->setfieldwidth(10,85);
            $this->setfieldwidth(20);
            $this->textbox(number_format_indian($this->billsummary['SLIPTONNAGE'],3),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',10,'','','','');
            $this->setfieldwidth(20);
           $this->setfieldwidth(20);
    ...
    { 
        ...
        else
        {
            ...
            $this->setfieldwidth(15);
            $this->textbox('ए.स्लिप',$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','');
            $this->setfieldwidth(20);
            $this->textbox($this->fieldslipcount,$this->w,$this->x,'N','L',1,'SakalMarathiNormal922',10,'','','','');
        ...
        $this->newrow(7);
        $this->setfieldwidth(20,10);
        //$this->textbox($group_row_1['HRTONNAGE'],$this->w,$this->x,'S','L',1,'SakalMarathiNormal922',10,'','','','B');
        //$this->setfieldwidth(20);
        //$this->textbox($group_row_1['TRTONNAGE'],$this->w,$this->x,'S','L',1,'SakalMarathiNormal922',10,'','','','B');
        $this->setfieldwidth(20);
        if ($group_row_1['BILLCATEGORYCODE']==1)
        {
        ...
        //$this->textbox('डिझेल फरक',$this->w,$this->x,'S','R',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(20);
        $this->textbox($group_row_1['DIESELRATEDIFFERENCEAMOUNT'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10,'','','','B');
        $this->setfieldwidth(20);
        $this->textbox('कमिशन',$this->w,$this->x,'S','R',1,'siddhanta',10,'','','','B');
        $this->setfieldwidth(20);
        ...
        $this->setfieldwidth(30);
        $this->textbox($group_row_1['NETGROSSAMOUNT'],$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',11,'','','','B');
        //$this->setfieldwidth(20);
        //$this->textbox($group_row_1['GROSSDEDUCTION'],$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',10,'','','','B');
        //$this->setfieldwidth(25);
        //$this->textbox($group_row_1['NETAMOUNT'],$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',10,'','','','B');
        $this->newrow(7);
        $this->hline(10,195,$this->liney,'D');
    }
    ...
    { 
        ...
        if ($this->servicetrhrcategorycode != 6)
        {
            $detail_query_1 = "select t.deductioncode
            ,trim(trim(d.deductionnameuni) ||' '|| trim(t.dedseasonyear)||' '||trim(b.bankbranchnameuni)) dednameuni
            ,sum(t.deductionamount) deductionamount
            from HTBILLDEDUCTIONDETAIL t,htbillheader h
            ,deduction d
            ,bankbranch b
            where t.deductioncode=d.deductioncode 
            and t.branchcode=b.bankbranchcode(+) 
            and t.billtransactionnumber=h.transactionnumber 
            and h.billperiodtransnumber=".$group_row_1['BILLPERIODTRANSNUMBER']." 
            and h.transactionnumber=".$group_row_1['TRANSACTIONNUMBER'].
            " group by t.deductioncode
            ,trim(trim(d.deductionnameuni) ||' '|| trim(t.dedseasonyear)||' '||trim(b.bankbranchnameuni))";
        }
        elseif ($this->servicetrhrcategorycode == 6)
        {
            $detail_query_1 = "select t.deductioncode
            ...
            and t.billtransactionnumber=h.transactionnumber 
            and h.billperiodtransnumber=".$group_row_1['BILLPERIODTRANSNUMBER']." 
            and h.contractorcode=".$group_row_1['CONTRACTORCODE'].
            " group by t.deductioncode
            ,trim(trim(d.deductionnameuni) ||' '|| trim(t.dedseasonyear)||' '||trim(b.bankbranchnameuni))";
        }
        ...
        while ($detail_row_1 = oci_fetch_array($detail_result_1,OCI_ASSOC+OCI_RETURN_NULLS))
        {
            ...
            if ($srno%2) 
            {
                ...
                $this->textbox($srno.')',$this->w,$this->x,'N','L',1,'SakalMarathiNormal922',10,'','','','');
    ...
    {        
        ...
        $this->textbox('एकूण कपात->',$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','');  
        $this->setfieldwidth(28);
        $this->textbox($group_row_1['GROSSDEDUCTION'],$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',10,'','','','');  
        ...
        $this->textbox(NumberToWords($group_row_1['NETAMOUNT'],1),$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','');  
       
        $this->setfieldwidth(33);
        $this->textbox('निव्वळ देय ->',$this->w,$this->x,'S','L',1,'siddhanta',10,'','','','');
        $this->setfieldwidth(25);
