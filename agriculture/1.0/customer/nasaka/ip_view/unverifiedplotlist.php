<?php
        ...
        $result1 = $plantationheader1->unverifiedlist();
        if ($plantationheader1->Get_invalidid()==0)
        {
          ...
          while ($row1 = oci_fetch_array($result1,OCI_ASSOC+OCI_RETURN_NULLS))
          {
            ...
            {
              echo '<tr>';
              //echo '<td><a href="../ip_view/plantationheader.php?seasoncode='.fnEncrypt($row1['SEASONCODE']).'&plotnumber='.fnEncrypt($row1['PLOTNUMBER']).'&flag='.$flag_en.'">'.$row1['PLOTNUMBER'].'</td>';
              //echo '<td><input type="checkbox" name="plots[]" value="'.$row1["PLOTNUMBER"].'">'.$row1["PLOTNUMBER"].'</td>';
              echo '<td><a href="../ip_view/plantationheader.php?seasoncode='.fnEncrypt($row1['SEASONCODE']).'&plotnumber='.fnEncrypt($row1['PLOTNUMBER']).'&flag='.$flag_en.'">'.$row1["GUTNUMBER"].'</td>';
              //echo '<td>'.$row1["PLOTNUMBER"].'</td>';
