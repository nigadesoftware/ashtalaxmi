<?php
	...
	elseif (isset($_SESSION['LAST_ACTIVITY']) && (time() - $_SESSION['LAST_ACTIVITY'] > 180000)) {
    ...
    if (isset($_SESSION['usersname']))
    {
        ...
        echo '<div><img src="../img/agriculture.png" width="120px" height="31px"></div>';
        echo '<div><img src="../img/kadwa.jpg" width="35px" height="35px"></div>';
        echo '<div><label style="font-family:siddhanta;font-size:17px">'.$_SESSION['factoryname'].'</label></div>';
        echo '</header>';
        ...
        echo '<tr style="font-size:14px">';
                echo '<td><label class="userinfo"></br>Login User : </label></td>';
                echo '<td><label class="userinfo">'.$_SESSION['usersname'].'</br></label></td>';
                echo '</tr>';
        if (isset($_SESSION['responsibilityname']))
        {
            if ($_SESSION['responsibilityname']!="0")
            {
                ...
                echo '<tr style="font-size:14px">';
                echo '<td><label class="userinfo">Responsibility : </label></td>';
                echo '<td><label class="userinfo">'.$_SESSION['responsibilityname'].'</br></label></td>';
                echo '</tr>';
            }
        }
        if (isset($_SESSION['financialyear']))
        {
            if ($_SESSION['financialyear']!='0')
            {
                ...
                echo '<tr style="font-size:14px">';
                echo '<td><label class="userinfo">Season Year : </label></td>';
                echo '<td><label class="userinfo">'.$_SESSION['financialyear'].'</br></label></td>';
                echo '</tr>';
            }
        }
