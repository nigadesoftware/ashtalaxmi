<?php
    ...
    function swapp_database_settings(&$dbuser,&$host,&$dbname,&$pwd)
    {
        ...
        mysqli_query($connection,'SET NAMES UTF8');
        $connection ->autocommit(FALSE);
        $query = "select * from databaseserver d,miscustomermodules c where d.miscustomerid=c.miscustomerid and d.miscustomerid=".$_SESSION["factorycode"]." and dbuser='".$dbuser."'";
        $result = mysqli_query($connection,$query);
...
function getStringOfAmount($num) {
  ...
  function filllistcombo(&$connection,$sourcetable,$nameeng,$nameuni,$code,$iscompulsory=False,$found=false,$value='',$condition='')
  {
    echo '<tr>';
    if ($condition=='')
    {
        $query1 = "select {$code} as code,{$nameeng} as name_eng,{$nameuni} as name_unicode 
        from {$sourcetable} order by $nameuni" ;
    }
    else
    {
        $query1 = "select {$code} as code,{$nameeng} as name_eng,{$nameuni} as name_unicode 
        from {$sourcetable} where {$condition} order by $nameuni" ;
    }
    ...
    /*function label($fieldname,$englishlabel,$unicodelabel)
    {
        if ($_SESSION['lng']=="English")
        {
            echo '<tr>';
    ...
    function textcombobox($combosearchfield,$combocodefield,$englishlabel,$unicodelabel,$value='',$codevalue='',$width=300,$height=150,$iscompulsory=False,$iscompulsory=False,$found=false,$flag='',$js='')
    {
        textbox($combosearchfield,$englishlabel,$unicodelabel,$value,$width,$height,$iscompulsory,$isreadonly,$found,$flag,$js);
        if ($found==false)
        {
            echo '<tr>';
            echo '<td><input type="textbox" readonly="readonly" tabindex ="-1" style="font-size:12pt;height:'.$height.'px;width:'.$width.'px;" name="'.$combocodefield.'" id="'.$combocodefield.'"'.$js.'></td>';
            echo '</tr>';
        }
        else
        {
            echo '<tr>';
            echo '<td><input type="textbox" readonly="readonly" tabindex ="-1" style="font-size:12pt;height:'.$height.'px;width:'.$width.'px;" name="'.$combocodefield.'" id="'.$combocodefield.'" value="'.$codevalue.'"'.$js.'></td>';
            echo '</tr>';
        }
        
     
    }
    function textbox($fieldname,$englishlabel,$unicodelabel,$value='',$width=300,$height=150,$iscompulsory=False,$iscompulsory=False,$found=false,$flag='',$js='')
    {
        label($fieldname,$englishlabel,$unicodelabel);
        if ($iscompulsory==True and $flag!='Query')
        {
            if ($found==true)
            {
                echo '<tr>';
                echo '<td><input type="text" readonly="readonly" tabindex="-1" style="font-size:12pt;height:'.$height.'px;width:'.$width.'px;" name="'.$fieldname.'" id="'.$fieldname.'" value="'.$value.'"></td>';
            }
            else
            {
                echo '<tr>';
                echo '<td><input type="text" readonly="readonly" tabindex="-1" style="font-size:12pt;height:'.$height.'px;width:'.$width.'px;" name="'.$fieldname.'" id="'.$fieldname.'"></td>';
            }
        }
        else
        {
            ...
            {
                echo '<tr>';
                echo '<td><input type="text" style="font-size:12pt;height:'.$height.'px;width:'.$width.'px;" name="'.$fieldname.'" id="'.$fieldname.'" value="'.$value.'"></td>';
            }
            else
            {
                echo '<tr>';
                echo '<td><input type="text" style="font-size:12pt;height:'.$height.'px;width:'.$width.'px;" name="'.$fieldname.'" id="'.$fieldname.'" value="'.$value.'" '.$js.'></td>';
            }
        }
        if ($iscompulsory==True)
        {
 ...
 */
    function getresponsibilityid(&$connection,$addalter)
    {
        $phpfile = explode("?",basename($_SERVER["REQUEST_URI"]));
        $query = "select nvl(max(responsibilitycode),0) as responsibilitycode
                from nst_amrut_db.form t
                ,nst_amrut_db.formresponsibilitydetail r
                where t.formcode=r.formcode 
                and t.moduleid=".$_SESSION['mismoduleid'].
                " and responsibilitycode=".$_SESSION['responsibilitycode'].
                " and upper(formfile||'.php')=upper('".$phpfile[0]."')
                    and responsibilitytype=".$addalter;
        $result = oci_parse($connection, $query);
        $row = oci_execute($result);
        if ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
		{
            return $row['RESPONSIBILITYCODE'];
        }	
        else
        {
            return 0;
