...
class issueheader extends swappform
{
    ...
    Public $purpose;
    public $indenttransactionnumber;
    ...
    public function insert()
    {
        ...
        if ($this->issuenumber == '')
        {
            $query = "select nvl(max(issuenumber),0)+1 as issuenumber 
            from issueheader
            ...
            $this->issueprefixnumber = 'IND/'.$row["ISSUENUMBER"];
        ...
        $query = "
            ...
            ,".$this->invl($this->indenttransactionnumber,true,true)."
            )";
        $result = oci_parse($this->connection, $query);
        if (oci_execute($result,OCI_NO_AUTO_COMMIT))
        {
            
            $query ="insert into issueitemdetail(transactionnumber,serialnumber,itemcode,demandquantity)
            select ".$this->transactionnumber.",(select nvl(max(i.serialnumber),0) from issueitemdetail i 
            where i.transactionnumber=".$this->transactionnumber.")+rownum,d.itemcode,d.quantity
            from INDENTHEADER t,indentitemdetail d
    ...
    public function display()
	  {
            ...
            if ($this->issuenumber!='')
		    {
                if ($cond=='')
                {
                    $cond="t.issuenumber = ".$this->issuenumber;
                }
                else
                {
                        $cond=$cond." and t.issuenumber = ".$this->issuenumber;
                }
		    }
            if ($this->issuedate!='')
		    {
			    if ($this->issuedate!='')
                {
                    $issdate = DateTime::createFromFormat('d/m/Y',$this->issuedate)->format('d-M-Y');
                }  
                if ($cond=='')
                {
                    $cond="t.issuedate = '".$issdate."'";
                }
                else
                {
                    $cond=$cond." and t.issuedate = '".$issdate."'";
                }
		    }
            if ($cond!='')
		    {
		        $query = "select t.* from issueheader t where ".$cond." order by transactionnumber";
			    $result = oci_parse($this->connection, $query);
                $r = oci_execute($result);
			    return $result;
		    }
            ...
            {
                $query = "select t.* from issueheader t order by transactionnumber";
                $result = oci_parse($this->connection, $query);
                $r = oci_execute($result);
                return $result;
            }
    ...
    public function update()
    {
        ...
        ,indenttransactionnumber= ".$this->invl($this->indenttransactionnumber,true,true)."
        where 
    ...
    public function fetch()
    {
        ...
        { 
                ...
                $this->issuetoemployeecodetextcombo = $this->textcomboname($connection=$this->connection,$table='EMPLOYEE',$datafield='EMPLOYEECODE',$datafieldvalue=$row['ISSUETOEMPLOYEECODE'],$engfield='EMPLOYEENAMEENG',$unicodefield='EMPLOYEENAMEUNI');
                ...
                $this->indenttransactionnumber = $row['INDENTTRANSACTIONNUMBER'];
                $this->indenttransactionnumbertextcombo = $this->textcomboname($connection=$this->connection,$table='INDENTHEADER',$datafield='TRANSACTIONNUMBER',$datafieldvalue=$row['TRANSACTIONNUMBER'],$engfield='INDENTPREFIXNUMBER',$unicodefield='INDENTPREFIXNUMBER');
                $this->found=true;
                return true;
        }
        else
