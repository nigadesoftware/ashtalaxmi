<?php
    ...
    if ($isplus==0)
    {
        $query = "select nst_db.englastwordnumber('".$searchTerm."') as wordnumber from dual";
        $result = oci_parse($connection, $query);
        $r = oci_execute($result);
        if ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
        {
            $query1 = "select nst_db.engword('".$searchTerm."',".$row['WORDNUMBER'].") as word from dual";
            ...
            if ($row1 = oci_fetch_array($result1,OCI_ASSOC+OCI_RETURN_NULLS))
            {
                ...
                $query2 = "select T.NAME_UNI,T.NAME_ENG from nst_db.name t
