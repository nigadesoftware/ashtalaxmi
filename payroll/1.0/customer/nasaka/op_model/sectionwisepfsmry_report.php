...
{	
    ...
    {
        $this->groupfield1='BASICDAAMOUNT';
        $this->groupfield2='PFABLEAMOUNT';
        $this->groupfield3='EMPLOYERPFAMOUNT';
        $this->groupfield4='EMPLOYERPENSIONAMOUNT';
    ...
    function group()
    {
        ...
        $group_query_1 ="
        select 
        row_number() over( order by et.sectioncode)sr_no
        ,et.sectioncode
        ,s.sectionnameuni
        ,sum(h.basicdaamount)basicdaamount
        ,sum(h.pfableamount)pfableamount
        ,sum(h.employerpfamount)employerpfamount
        ,sum(h.employerpensionamount)employerpensionamount
        from SALARYTRANSACTIONHEADER h,employeetransaction et
        ,salaryperiod p,section s
        where --{$cond}
         h.salaryperiodtransactionnumber=p.transactionnumber
        and h.employeecode=et.employeecode
        and et.employeecategorycode=p.employeecategorycode
        and et.sectioncode=s.sectioncode
        and p.employeecategorycode=et.employeecategorycode
        and h.employerpfamount>0
        group by et.sectioncode,s.sectionnameuni
        order by sr_no
       ";
    ...
    function detail_1(&$group_row_1)
    {
        ...
        $this->setfieldwidth(25);
        $this->textbox($group_row_1['SECTIONCODE'],$this->w,$this->x,'S','C',1,'SakalMarathiNormal922',11);
        $this->vline($this->liney-1,$this->liney+7,$this->x+$this->w);
        
        $this->setfieldwidth(45);
        $this->textbox($group_row_1['SECTIONNAMEUNI'],$this->w,$this->x,'S','L',1,'siddhanta',11);
        $this->vline($this->liney-1,$this->liney+7,$this->x+$this->w);
    ...
    function reportfooter()
    {
        ...
        $this->setfieldwidth(25);    
        $this->vline($this->liney-1,$this->liney+7,$this->x);
        $this->textbox(number_format($this->showgroupsummary(0,'BASICDAAMOUNT'),0,'.',''),$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',11,'','','','B');
        $this->vline($this->liney-1,$this->liney+7,$this->x+$this->w);
        
        $this->setfieldwidth(25);    
        $this->vline($this->liney-1,$this->liney+7,$this->x);
        $this->textbox(number_format($this->showgroupsummary(0,'PFABLEAMOUNT'),0,'.',''),$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',11,'','','','B');
        $this->vline($this->liney-1,$this->liney+7,$this->x+$this->w);

        $this->setfieldwidth(25);    
        $this->vline($this->liney-1,$this->liney+7,$this->x);
        $this->textbox(number_format($this->showgroupsummary(0,'EMPLOYERPFAMOUNT'),0,'.',''),$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',11,'','','','B');
        $this->vline($this->liney-1,$this->liney+7,$this->x+$this->w);

        $this->setfieldwidth(25);    
        $this->vline($this->liney-1,$this->liney+7,$this->x);
        $this->textbox(number_format($this->showgroupsummary(0,'EMPLOYERPENSIONAMOUNT'),0,'.',''),$this->w,$this->x,'S','R',1,'SakalMarathiNormal922',11,'','','','B');
        $this->vline($this->liney-1,$this->liney+7,$this->x+$this->w);
