...
{	
   public $summary;
    ...
    {
        $this->groupfield1='DEDUCTIONAMOUNT';
    ...
    { 
       ...
       // $this->textbox($this->ftransactionnumber['FARMERCOUNT'],175,10,'S','C',1,'siddhanta',13);
         ...
         $this->newrow(10);
        $this->textbox('कपात वाईज कर्मचारी यादी माहे-'.$this->monthname.' '.$_SESSION['yearperiodcode'],150,10,'S','C',1,'siddhanta',13);
    ...
    function group()
    {
        ...
        {
            if ($cond=="")
                $cond="sa.deductioncode=".$this->deductioncode;
            else
                $cond=$cond." and sa.deductioncode=".$this->deductioncode;
        }
        $group_query_1 ="
        select         
        sa.deductioncode
        ,ea.deductionnameuni
        ,row_number() over(partition by sa.deductioncode order by h.employeecode)sr_no
        ,h.employeecode
        ,em.employeenameuni
        ,sa.deductionamount
       from salarytransactionheader h,salaryperiod p
       ,employeetransaction e,employeemaster em
       ...
       and h.employeecode=em.employeecode
    ...
    { 
      ...
      $this->textbox('कपात नाव: '.$group_row_1['DEDUCTIONNAMEUNI'],$this->w,$this->x,'S','L',1,'siddhanta',12,'','','','B');  
    ...
    function detail_1(&$group_row_1)
    {
        ...
        $this->summary["payment"]+=$group_row_1['DEDUCTIONAMOUNT'];
    ...
    { 
        ...
        $this->textbox('एकूण ',$this->w,$this->x,'N','R',1,'siddhanta',11,'','','','');
        $this->vline($this->liney-1,$this->liney+7,$this->x+$this->w);
     
        $this->setfieldwidth(25);    
        $this->vline($this->liney-1,$this->liney+7,$this->x);
       $this->textbox(number_format($this->summary["payment"],0,'.',''),$this->w,$this->x,'N','R',1,'SakalMarathiNormal922',11,'','','','');
        $this->vline($this->liney-1,$this->liney+7,$this->x+$this->w);
