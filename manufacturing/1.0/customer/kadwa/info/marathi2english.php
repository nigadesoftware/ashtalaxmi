...
function mar2eng($marname)
{
	...
	for ($i=0;$i<strlen($marname);$i=$i+3)
	{
		...
		switch (substr($marname,$i,3))
		{
			...
			case 'इ': 
				...
				break;
			case 'ई':
				if ($curakshar ==$totalakshar)
				$retstr =substr($retstr, 0,strlen($retstr)-1).'i';
				else
				$retstr =substr($retstr, 0,strlen($retstr)-1).'ee';
				break;
			case 'ि': 
				$retstr =substr($retstr, 0,strlen($retstr)-1).'i';
				break;
			...
			case 'ऊ':
				...
				$retstr =substr($retstr, 0,strlen($retstr)-1).'oo';
				break;
			case 'ु':
				$retstr =substr($retstr, 0,strlen($retstr)-1).'u';
				break;
		...
		/* if ($totalakshar==3)
		{
			if ($curakshar==3)
			{
				...
				if (substr($retstr,strlen($retstr)-1,1)=='a' and substr($marname,$i,3)!=='ा')
				{
		...
		if ($totalakshar==4)
		{
			if ($curakshar==2 and isconsonent(substr($marname,$i+3,3))==1 and (substr($marname,$i,3)!='ह'))
			{
				//echo '!'.countlastardhakshar($marname,$curakshar);
	...
	$retstr = preg_replace('/ksh/','x',$retstr,-1);
...
function countlastardhakshar($marname,$curcnt)
{
	...
	for ($i=0;$i<strlen($marname);$i=$i+3)
	{
		...
		if (substr($marname,$i,3)=='्' and substr($marname,$i+6,3)=='')
		{
			$acnt++;
		}
	}
